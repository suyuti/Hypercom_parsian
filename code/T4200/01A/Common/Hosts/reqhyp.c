	...
	if ( NULL != pBitMap_rec )
	{
		...
		
		{
			...
			{				
                ...
                // Preauth Void i?leminin mesaj type'i 100 olmaly .. 
...
static Bool MovProcHyp( void )
{
	...
	if ( TRREQ.TRKEY == RUPLOAD )
	{
        ...
        else if ( TRREQ.TRSTATUS[0] & TS1_REV )// reversal ise özel p.c. 
	{       
        switch (TRREQ.TRKEY)
        {
        case REFUND:
	...
	else if ( VOIDTRAN == TRREQ.TRKEY ) // iptal i?lemi ise özel p.c 
	{   
...
//! Each entry respresents a single move item. 
//! All items are saved in the Send Buffer.    
const struct move_rec Move_Tab[] = { //! Primary Account Number           
	{2, MovPan},    //! Processing Code                  
	{3, MovProcHyp},    //! Amount, Transaction              
	{4, MovTotAm},    //! Systems Trace Audit Number       
	{11, MovStan},    //! Time, Transaction                
	{12, MovTime},    //! Date, Transaction                
	{13, MovDate},    //! Date, Expiry                     
	{14, MovExpdYYMM},    //! POS Entry Mode                   
	{22, MovPose},    //! Network International Identifier 
	{24, MovNII},    //! POS Condition Code               
	{25, MovPocc},    //! Track 2 Data                     
	{35, MovTrk2},    //! Retrieval Reference Number       
	{37, MovRRN},    //! Auth Id Response code  (Approval code)           
	{38, MovAuth},    //! Response code            
	{39, MovResp},    //! Terminal Id                      
	{41, MovTID},    //! Acceptor Id                      
	{42, MovAccID},    //! PIN number                       
	{52, MovPin},    //! Tip Amount                       
	{54, MovTip},
#ifdef MAKE_EMV
    {55, MovICC}, //! ICC data
#endif
...
extern OS_RETURN_VALS ReqHyper( void )
{
	...
	if ( NULL != pBitMap_rec )
	{
		...
		if ( True == ReqPackMsg( pBitMap_rec->bitmap ) )
		{
            ...
            if( SUPER_VARIABLES_TAB.dontSendPosTPDU )
            {
                if( SENDBUF[0] == TPDUID_APP )
                {
                     memmove(&SENDBUF[0], &SENDBUF[TPDU_SZE], TPDU_SZE);
                     SendMsgLen -= TPDU_SZE;
                }
            }
