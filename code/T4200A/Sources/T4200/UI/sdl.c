...
#ifdef MAKE_FARSI
    // Apply Arabic digits on inputbox
    // Check if Farsi language selected    
    int i;
    if( LOCTAB.LANGID == LNG_FA )
    {
        // Allocatre memory for farsi representation buffer
        char* pFarsiBuf = malloc ( sizeof( InputBoxBuffer ) );

        memset( pFarsiBuf, 0, sizeof( InputBoxBuffer ) );
        memcpy( pFarsiBuf, InputBoxBuffer, sizeof( InputBoxBuffer ) );  

        // Remap FARSI numbers 0x30->0x80...0x39->0x89
        for( i=0; i<sizeof( InputBoxBuffer ); i++ )
        {
            if( (pFarsiBuf[i] >= '0') && (pFarsiBuf[i] <= '9'))
            {
                pFarsiBuf[i] += 0x50;
            }
        }

        len = StrLn( pFarsiBuf, sizeof( InputBoxBuffer ) );
        maxlen = ( UBYTE ) ( ibTextArea.Rct.width / charsz.Width );
        if ( len > maxlen )
        {
            // chop off start of text
            memcpy( TextAreaBuffer, &pFarsiBuf[maxlen - len],
				( UWORD ) ( maxlen + 1 ) );
            pTxt = TextAreaBuffer;
        }
        else
