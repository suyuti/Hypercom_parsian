...
//--------------------------------------------
		if ( (KeyVal == EVT_BUTTON_UP) ||
			 (KeyVal == EVT_BUTTON_DOWN) ||
...
//------------ Main Menu Processing ---------
			...
			{//Simulate up,down,select- MENU system
				...
				if (KeyVal == EVT_BUTTON_DOWN) //Down
				{
                ...
                else if (KeyVal == EVT_BUTTON_UP) //Up
				{
				...
				if ( (KeyVal == EVT_BUTTON_SELECT) || ( KeyVal == ENTER_KY ) )
				{
                    if ( MenuItemCounter < 5 )
                    {
                        MenuX = B10_M_LEFT;
