...
//	InitCryptoProc();
    ...
    if ( bInitOk == False )
    {
        // Show notification to user and terminate application
        SDK_GuiClearScr(  );
        SDK_GuiCenterText( 1, "Error:" );
        SDK_GuiCenterText( 2, "configuration data" );
        SDK_GuiCenterText( 3, "not present" );
        SDK_GuiDispAll( );
	...
	if ( !JournalValidate() )
	{
        ...
        if( !AppFirstRun )
        {
            // Notify user "Transaction Memory" if not the first application run
            // JournalValudate returns 0 on first run because journal is not exist
...
//###			if ( TERM.TERMOpt4 & TM4_ICC )
			...
			inputtype = read_io( device_map, 1, &bPinCardStatus, &DevFlag );	// Wait for input
			...
			SDK_RelinqCPU();
			SDK_Wait ( 10 * TENMS );  // wait 100ms to reduce the idle thread cpu usage
		}
