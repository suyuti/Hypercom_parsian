...
extern Bool JournalReWrite( struct trandata_rec * pTranBuf )
{
    ...
    if( !pTranBuf->TRJRNIDENT )
    {
        ...
        //MoveIt( JSREC.JSINV, pTranBuf->TRSTAN, S_TRSTAN);
...
static UBYTE SelMatch(  )
{
    ...
    //if ( !NullComp( ( char * ) JSREC.JSINV, S_TRSTAN ) )
    {
    ...
    // Added for Parsian
    // Parsian'da sliplerde Invoice Number yerine STAN yazilmaktadir.
    // Bu yüzden arama STAN ile olabilir.
    if ( !NullComp( (char *) JSREC.JSSTAN, S_TRSTAN))
    {
        if ( memcmp( JSREC.JSSTAN, JRBUF.TRDATA.TRSTAN, S_TRSTAN ) )
            return ( False );
        if ( memcmp( JSREC.JSSTAN, JRBUF.TRDATA.TRLTSTAN, S_TRSTAN ) )
            return ( False );
    }
