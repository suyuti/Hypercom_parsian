...
extern void oldAPP1SALE( void )
{
    ...
    if (!((TRINP.TRISPTR->ISOPT2) & IS2_CAPTURE)) {   /* Don't Capture transactions? */
        if (!AcctCheck(  ))
            return;
...
{              
    ...
    if (!(TCONF.TCOPT3 & TC3_CLPREPERMIT)) {
        //ShowInfoMsg(TerminalNotAllowed,N_Error);
        SDK_Wait(ONESECOND * 3);
        return;
    }
...
extern void CommonTran( void )  
{
    ...
    //    // must be converted to Jalali
    //    AscHex( TRINP.TRTIME,       ( char * ) &RtcBuf[6], S_TRTIME );
    //    AscHex( TRINP.TRJALALIDATE, ( char * ) &RtcBuf[2], S_TRDATE );
    //    AscHex( TRINP.TRJALALIYEAR, ( char * ) &RtcBuf[0], S_TRYEAR );
    //}
    //else {
        AscHex( TRINP.TRTIME, ( char * ) &RtcBuf[6], S_TRTIME );
    ...
    //GetJelaliDateTime(TRINP.TRDATE, TRINP.TRJALALIDATE);
...
#else
/*
Not: tempBuf'ýn ne oldugu belli degildi.
  do {
