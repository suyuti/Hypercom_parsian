...
extern int GetStringLenghtInPixel(const char* str, int len);
extern int LayoutVariableWidthChars(int maxPix, const char *line, int len,  int fillChar1, int fillChar2);
...
extern void PrintInit( void )
{
    ...
    memset( LineBuff, 0xFF, BUFSZE + 1 );	// Clear the printer line buffer // \xFE bir pixellik bosluk.
...
extern Bool PrintLINE( void )
{
	...
	if ( PrintSubLine(  ) )
	{
        ...
        LineSize = LayoutVariableWidthChars(maxDotPerLine, (const char *)LineBuff, LineSize,  0xFF, 0xFE);
        ...
        //memset( LineBuff, ' ', BUFSZE + 1 );	// Clear the printer line buffer
        memset( LineBuff, 0xFF, BUFSZE + 1 );	// Clear the printer line buffer
	}
	return ( Status );
}
...
static Bool PrintSubLine( void )
{
    ...
    size = GetStringLenghtInPixel((const char *)BUF3, TrueLength);
    ...
    case FD_JEJALI_TEXT:
        {
            ...
            //tmp_t is now in Jelali

            // Get Month String
            strncpy(tmp_t2, &tmp_t[2], 2);
            tmp_t2[2] = '\0';
            int monthVal = atoi(tmp_t2);
            memcpy(BUF2, Month12[monthVal], 3);
...
extern int GetStringLenghtInPixel(const char* str, int len)
{
    ...
    int i = 0;
    int j = 0;
    int size = sizeof (Farsi_prnCodePage_table) / sizeof(Farsi_prnCodePage_record);
    //while (str[i] != '\0') {
    while (i < len) {
        for (j = 0; j < size; ++j) {
            if (Farsi_prnCodePage_table[j].ASCII == str[i]) {
                if (Farsi_prnCodePage_table[j].fpi_aln_lft != NULL) {
                    retVal += Farsi_prnCodePage_table[j].fpi_aln_lft->CharWdth;
                }
                break;
            }
        }
...
extern int LayoutVariableWidthChars(int maxPix, const char *line, int len,  int fillChar1, int fillChar2)
{
    int part1Len        = 0;    // Sola dayali kisim string uzunlugu
    int part2Len        = 0;    // Saga dayali kisim string uzunlugu
    int fillPosBegin    = 0;    // Bosluk baslangic pozisyonu
    int fillPosEnd      = 0;    // Bosluk bitis pozisyonu
    int sizeOfPart1     = 0;    // Sola dayali kisim pixel uzunlugu
    int sizeOfPart2     = 0;    // Saga dayali kisim pixel uzunlugu
    int gapInPixel      = 0;    // Bosluk pixel uzunlugu
    int fillChar1Count  = 0;    // Genis (16p) karakter bosluk sayisi
    int fillChar2Count  = 0;    // Dar   (1 p) karakter bosluk sayisi
    int i               = 0;
    int pos             = 0;

    char buffer[400] = "";

    while (line[fillPosBegin] != fillChar1 && fillPosBegin <= len) {
        ++fillPosBegin;
        ++part1Len;
    }

    fillPosEnd = fillPosBegin;
    while (line[fillPosEnd] == fillChar1 && fillPosEnd <= len) {
        ++fillPosEnd;
    }

    part2Len = len - fillPosEnd;

    sizeOfPart1 = GetStringLenghtInPixel(line, part1Len);
    sizeOfPart2 = GetStringLenghtInPixel(line + fillPosEnd, part2Len);

    if (PRFLAG & PF_DBLE) {
        sizeOfPart1 <<= 1;
        sizeOfPart2 <<= 1;
    }
    gapInPixel = maxPix - sizeOfPart1 - sizeOfPart2;

    fillChar1Count = gapInPixel / 16;
    fillChar2Count = gapInPixel % 16;
    ...
    memcpy(buffer, line, part1Len);
    pos = part1Len;
    for (i = 0; i < fillChar1Count; ++i) {
        memset(buffer + pos, fillChar1, 1);
        ++pos;
    }
    for (i = 0; i < fillChar2Count; ++i) {
        memset(buffer + pos, fillChar2, 1);
        ++pos;
    }
    memcpy(buffer + pos, line + fillPosEnd, len - part2Len);

    memcpy(line, buffer, pos + part2Len);
    return (part1Len + part2Len + fillChar1Count + fillChar2Count);
}
