			  ...
			  int ksn_size, unsigned char key_id, int doubleKey, unsigned char* workingKey )
{
			...
			m_pVPPdata.type = ( VPP_SessionType )action;
			m_pVPPdata.pAD = workingKey;		// Pointer to Additional Data
			m_pVPPdata.sizeAD = doubleKey;		// Size of the Additional Data buffer
			m_pVPPdata.timeout = 99;			// Timeout value (seconds), 5 to 200 seconds.
			m_pVPPdata.indexKey = key_id;		// Index for desired key in the key collection.
...
VPP_EVENTS VPPGetPin( char Maxlen, char* screen_buf, char* dest, Call_Back UpdateInput, unsigned char *sw1sw2 )
{
    ...
    UBYTE pinlen = 0;
        ...
        while ( 1 )
        {
            ...
            {
                    // The input buffer is full.  The entered PIN is too large.
                    vpp_app_events = VPP_BufferFull;
                    break;
            }
