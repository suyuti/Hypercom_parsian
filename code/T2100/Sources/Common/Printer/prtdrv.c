...
#define BUFSZE  46				// MAXIMUM EDIT FIELD SIZE
								// (NEEDS TO BE BIG ENOUGH FOR WIDEST PRINTER)

#define S_TRK1NAME 26			// Size of Name in Track 1 Data

#define RF_OTRUE    0x01		// RE_Flag bit: Outer nested IF codition is true
#define RF_ITRUE    0x02		// RE_Flag bit: Inner IF condition is true
#define RF_ELSE     0x04		// RE_Flag bit: Else token is allowed
#define RF_ATRUE    0x08		// RE_Flag bit: True case has already occurred
...
{
	UBYTE Code;					// Command code
	UBYTE Operands;				// Number of operands
	Bool( *Func ) ( void );	// Routine for this command
	UBYTE Flags;				// Flags for this command
};


UBYTE LOCBUF[S_AQVISATID]; 	/* Buffer for acquirer and issuer data */
							/* the size is the size of the longest field in AQTAB and ISTAB */

// Flags for CommandTableStruct
#define CMD_TF      0x01		// Process routine even when False
#define CMD_VAL     0x02		// Multiple Char, read the value
#define CMD_FOR     0x02		// Field - adjust format
#define CMD_STR     0x04		// Field - adjust the start
#define CMD_LEN     0x08		// Field - adjust length
#define CMD_RESJ    0x10		// Reset center & right justify flags
#define CMD_DMPB    0x20		// Dump character buffer
#define CMD_POFF    0x40		// Used to process when True but print off
...
extern Bool TestTRN_INSTAL( void );
extern Bool TestTRN_INSREF( void );
...
extern Bool TestTRN_Discount( void );
extern Bool TestTRN_Point( void );
extern Bool TestTRN_AllPoint( void );
extern Bool TestTRN_Loyalty( void );
extern Bool TestTRN_OnusCard( void );
extern Bool TestTRN_UsedPin( void );

extern Bool TestFlagSecondCopy( void );
extern Bool TestFlagPanTrunc( void );
extern Bool TestFlagPrintTime( void );
...
UBYTE PRLINE;					// Print Line Number

struct FLDDEF *(*PRVECT[8])( void );/* Custom Function vector Pointers */

struct trandata_rec *TRLOC;		// Transaction Buffer Location

UBYTE *CMDLOC;					// Current pointer to PCL report
UBYTE *CMDEND;					// Pointer to end of PCL report

UWORD CMDMORE;					// Length of report
UBYTE *CMDSTART;				// Start of report
PAGE_NUMBERS CMDPAGE;			// page that contain report token

UBYTE PresetPRFLAG;				// used to set PRFLAG for PCL operations
...
char TextGetBuf[50];
struct trandata_rec TRPRT;		// Default Print Transaction Data


//=============================================================================
// Private data definitions
//=============================================================================
struct FLDDEF *pFieldDef;		// Pointer to Field Definition
static UBYTE *pFieldData;		// Address of the Field
static UBYTE FieldLen;			// Length of the Field

// Edited Field Related Items
static char *FLD_ADR;			// Starting Address of the Edited Field
static UBYTE FLD_LEN;			// The Length of the Edited Field

static UBYTE PrintPos;			// Current position within print sub-line
static UBYTE PRBUF3LN;			// Length of the sub-line to print


static UBYTE PRJUST;			// Print Line Text Justification
...
static UBYTE LineLoc;			// Current location within print line
static UBYTE LineSize;			// Current length of print line

static UBYTE HOLDAMT[6];
static UBYTE HOLDSGN;

// PCL interpreter state variables
static UBYTE PCL_Error;			// Error in the PCL report
static BYTE Level;				// Re-entrancy level
static UBYTE CmdFlags;			// Copy of command flags from CommandTable
static UBYTE RE_Flag;			// Re-entrant interpreter flags
...
//-----------------------------------------------------------------------------
//  PrintInit       Print Initialization
//
//  Parameters:     None
//
//  Global Inputs:
...
//-----------------------------------------------------------------------------
//  PrintDriver     Print Command Driver
//
//  Parameters:     enum REPORTTYPE Report              report Type.
//
//                  struct trandata_rec * pTranData     Pointer to transaction
//                                                      data structure.  If 
//                                                      NULL,the default
//                                                      transaction data 
//                                                      structure TRPRT will be
//                                                      used.
//  Global Inputs:
//
...
//-----------------------------------------------------------------------------
//  PrintDriverCommon   Print Command Driver commonly called by either
//                      PrintDriver or PrintCapture.
//
//  Parameters:         enum REPORTTYPE Report      report type
//
//  Global Inputs:
...
//-----------------------------------------------------------------------------
//  FindReport      Find a PCL report
//
//  Parameters:     enum REPORTTYPE report      report type
//
//  Global Inputs:
//
//  Returns:        Bool        True if the report was found, else False
//
//  Notes:
//
//-----------------------------------------------------------------------------
...
//  Returns:        None
//
//  Notes:          Error code will be put in PCL_Error.
//
//-----------------------------------------------------------------------------
...
{
	UBYTE token;
	const static struct CommandTableStruct *cmd;
...
//-----------------------------------------------------------------------------
//  PeekToken       Return the next PCL token, but do not advance the pointer.
//
//  Parameters:     None
//
//  Global Inputs:
//
//  Returns:        UBYTE       token, or NO_CMD if no more tokens
//
//  Notes:
//
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
//  PrintItem       Buffer the specified item for printing
//
//  Parameters:     char *Item      data to be buffered
//
//                  UBYTE Length    length of the data to be buffered.
//
//  Global Inputs:
//
//  Returns:        Bool            True  - okay, 
...
//-----------------------------------------------------------------------------
//  PrintMLTCHR     Print a given character a given number of times
//
//  Parameters:     None
//
//  Global Inputs:
...
extern Bool PrintMLTCHR( void )
{
	...
	if ( ( PrintPos + number ) > MaxCols )
		number = MaxCols - PrintPos;	// Print as many as will fit
...
//-----------------------------------------------------------------------------
//  PrintLINE       Print the current line of data
//
//  Parameters:     None
//
//  Global Inputs:
...
//-----------------------------------------------------------------------------
//  FlushBuffers    Flush all print buffers with/without formfeed
//
//  Parameters:     char appendChar     character to ouput, or NULL if none
//
//  Global Inputs:
//
//  Returns:        Bool        True  - okay, 
...
//-----------------------------------------------------------------------------
//  PrintFORM       Print the current line of data with formfeed
//
//  Parameters:     None
//
//  Global Inputs:
...
extern Bool PrintFORM( void )
{
	...
	return ( FlushBuffers( '\f' ) );
}

//-----------------------------------------------------------------------------
//  PrintFormFeed   Print the current line of data with formfeed
//
//  Parameters:     None
//
//  Global Inputs:  None
...
//-----------------------------------------------------------------------------
//  PrintIt         Send data to the printer
//
//  Parameters:     char * SrcPtr   contains the data to be printed
//
//                  UWORD DataLen   is the size of the data
//
//                  Bool ForceIt    flush the output buffer to printer when
//                                  set to 1(Non-zero).None
//
//  Global Inputs:
//
//  Returns:        Bool        False   - Printing failed
//                              True    - printing successfully done
//
//  Notes:
//
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
//  PrintErrDetect  Check for printer hardware errors
//
//  Parameters:     None
//
//  Global Inputs:
...
//                              False - error detected.
//
//  Notes:          field - field to be printed.
//                  format - field reformat flag.
...
//-----------------------------------------------------------------------------
//  PrintNORMAL     Set printing to Normal
//
//  Parameters:     None
//
//  Global Inputs:
...
//-----------------------------------------------------------------------------
//  PrintDOUBLE     Set printing to Double Wide Characters
//
//  Parameters:     None
//
//  Global Inputs:
...
//-----------------------------------------------------------------------------
//  SelectPrinterFont   Select the printer font
//
//  Parameters:         UBYTE   fontID Font ID (0 or 1)
//
//  Global Inputs:
//
//  Returns:            None
...
//-----------------------------------------------------------------------------
//  PrintWIDTH      Set printer font width
//
//  Parameters:     None
//
//  Global Inputs:
...
//-----------------------------------------------------------------------------
//  SelectPrinterFont_Width     Select the largest printer font such that the 
//                              characters/line is >= the passed parameter.
//
//  Parameters:                 UBYTE lineWidth     Desired width
//
//  Global Inputs:
//
//  Returns:                    None
...
//-----------------------------------------------------------------------------
//  PrintLEFT       Print the next field(s) Left Justified on Line
//
//  Parameters:     None
//
//  Global Inputs:
...
//-----------------------------------------------------------------------------
//  PrintCNTR       Print the next field(s) Center Justified on Line
//
//  Parameters:     None
//
//  Global Inputs:
...
//-----------------------------------------------------------------------------
//  PrintRGHT       Print the next field(s) Right Justified on Line
//
//  Parameters:     None
//
//  Global Inputs:
...
//-----------------------------------------------------------------------------
//  PrintPOSLFT     Print the next field at a starting position 
//                  within the line (Left Justified)
//
//  Parameters:     None
//
//  Global Inputs:
...
extern Bool PrintPOSLFT( void )
{
	...
	if ( PrintSubLine(  ) )		// Move print sub-line to print line
	{
		if ( StartPos <= MaxCols )	// See if new starting position is valid
...
//-----------------------------------------------------------------------------
//  PrintPOSRGHT    Print the next field at an ending position 
//                  within the line (Right Justified)
//
//  Parameters:     None
//
//  Global Inputs:
...
//-----------------------------------------------------------------------------
//  PrintPOSCNTR    Position cursor to designated location and print the
//                  following text centered at that point
//
//  Parameters:     None
//
//  Global Inputs:
...
//-----------------------------------------------------------------------------
//  PrintReport     Recursive call to PCL interpreter to print another report
//
//  Parameters:     None
//
//  Global Inputs:
...
//-----------------------------------------------------------------------------
//  PrintENDIF      ENDIF - End of IF or ELSE clause
//
//  Parameters:     None
//
//  Global Inputs:
...
//-----------------------------------------------------------------------------
//  PrintELSE       Logical ELSE (must follow IF)
//
//  Parameters:     None
//
//  Global Inputs:
...
//-----------------------------------------------------------------------------
//  PrintELSEIF     Logical ELSE IF (must follow IF)
//
//  Parameters:     None
//
//  Global Inputs:
...
//-----------------------------------------------------------------------------
//  PrintOR         Logical OR token should not occur outside of an IF test
//
//  Parameters:     None
//
//  Global Inputs:
...
//-----------------------------------------------------------------------------
//  PrintAND        Logical AND token should not occur outside of an IF test
//
//  Parameters:     None
//
//  Global Inputs:
...
//-----------------------------------------------------------------------------
//  ComplexTest     Complex test for True or False according to the
//                  arguments of an IF or ELSEIF command
//
//  Parameters:     None
//
//  Global Inputs:
...
//-----------------------------------------------------------------------------
//  SimpleTest      Simple test for True or False according to one
//                  argument of an IF or ELSEIF command
//
//  Parameters:     None
//
//  Global Inputs:
...
extern Bool SimpleTest( void )
{
	...
	else
	{
		switch ( token )
		{
			...
			case TRN_INSTAL:
				result = TestTRN_INSTAL();   // INSTALLMENT 
				break;
			...
			case TRN_REFD:
				...
				break;
                        ...
                        case TRN_INSREF:
                                result = TestTRN_INSREF(  );	// INSTALLMENT REFUND
				break;                              
			...
			case TRN_NOSIG:
				...
				// CONDITION TESTS
                        case CND_DISCOUNT:
                                result = TestTRN_Discount();
                                break;

                        case CND_POINT:
                                result = TestTRN_Point();
                                break;

                        case CND_ALLPOINT:
                                result = TestTRN_AllPoint();
                                break;
                         
                        case CND_LOYALTY:
                                result = TestTRN_Loyalty();
                                break;

                        case CND_ONUSCARD:
                                result = TestTRN_OnusCard();
                                break;

                        case CND_USEDPIN:
                               result = TestTRN_UsedPin();
                               break;
                        
                        case CND_SECONDCOPY:
                              result = TestFlagSecondCopy();
                               break;

                        case CND_PANTRUNC   :
                              result = TestFlagPanTrunc();
                               break;

                        case CND_PRINTTIME     :
                               result = TestFlagPrintTime();
                               break;
...
//-----------------------------------------------------------------------------
//  PrintSignature      Print a signature
//
//  Parameters:         None
//
//  Global Inputs:
...
//-----------------------------------------------------------------------------
//  PrintGraphics   Print bitmapped graphics
//
//  Parameters:     None
//
//  Global Inputs:
...
//-----------------------------------------------------------------------------
//  PrintRed        Switch color ( if Verifone 250 printer )
//
//  Parameters:     None
//
//  Global Inputs:
...
//-----------------------------------------------------------------------------
//  TestTRN_SALE    Checks if Sale type transaction
//
//  Parameters:     None
//
//  Global Inputs:
//
//  Returns:        Bool        True  - Sale Transaction
//                              False - not a Sale Transaction.
//  Notes:
//
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
//  TestTRN_INSTAL    Checks if Installment type transaction
//
//  Parameters:     None
//
//  Global Inputs:
//
//  Returns:        Bool        True  - Installment transaction
//                              False - not a Installment transaction.
//
//  Notes:
//
//-----------------------------------------------------------------------------
...
//  Global Inputs:
//
//  Returns:        Bool        True  - Refund transaction
//                              False - not a Refund transaction.
//
//  Notes:
//
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
//  TestTRN_BALI    Checks if the transaction is a debit
//
//  Parameters:     None
//
//  Global Inputs:
//
//  Returns:        Bool    True  - Balance Inquiry transaction
//                          False - not Balance Inquiry transaction
//  Notes:
//
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
//  TestTRN_CRCD    Checks if the transaction is a credit card trans
//
//  Parameters:     None
//
//  Global Inputs:
//
//  Returns:        Bool        True  - credit card transaction
//                              False - not a credit card transaction
//  Notes:
//
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
//  TestTRN_DBCD    Checks if the transaction is a debit
//
//  Parameters:     None
//
//  Global Inputs:
//
//  Returns:        Bool        True  - DEBIT type transaction
//                              False - not DEBIT type transaction
//  Notes:
//
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
//  TestTRN_VOID    Checks if Voided Transaction
//
//  Parameters:     None
//
//  Global Inputs:
//
//  Returns:        Bool        True  - Voided Transaction
...
//-----------------------------------------------------------------------------
//  TestTRN_AVSVOID     Checks if AVS Voided Transaction
//
//  Parameters:         None
//
//  Global Inputs:
//
//  Returns:            Bool    True  - AVS Voided Transaction
//                              False - Transaction was not AVS Voided
//
//  Notes:
//
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
//  TestTRN_OPENT   Checks if Voided Transaction
//
//  Parameters:     None
//
//  Global Inputs:
...
//-----------------------------------------------------------------------------
//  TestCND_REJ     Checks if the Transaction was Rejected
//
//  Parameters:     None
//
//  Global Inputs:
//
//  Returns:        Bool        True  - Transaction was Rejected
//                              False - Transaction was not Rejected.
//
//  Notes:
//
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
//  TestCND_TIP     Checks if Tip Processing is on
//
//  Parameters:     None
//
//  Global Inputs:
//
//  Returns:        Bool        True  - Tip Processing is on
//                              False - Tip Processing is off
//
//  Notes:
//
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
//  TestCND_REPR    Checks if Transaction is being Reprinted
//
//  Parameters:     None
//
//  Global Inputs:
//
//  Returns:        Bool        True  - Transaction is being Reprinted
//                              False - Transaction is not being Reprinted.
//
//  Notes:
//
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
//  TestCND_PREP    Checks if Preprinting is in progress
//
//  Parameters:     None
//
//  Global Inputs:
//
//  Returns:        Bool        True  - Preprinting is in progress
//                              False - Preprinting is not in progress
//  Notes:
//
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
//  TestCND_SREQ    Checks if Signature not required
//
//  Parameters:     None
//
//  Global Inputs:
//
//  Returns:        Bool        True  - Signature is not required
//                              False - Signature is required.
//  Notes:
//
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
//  TestCND_MAN     Checks if the PAN was manually entered
//
//  Parameters:     None
//
//  Global Inputs:
//
//  Returns:        Bool        True  - PAN was manually entered
//                              False - PAN was not manually entered.
//  Notes:
//
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
//  TestTRN_NOSIG       Checks if signature is to be suppressed
//
//  Parameters:         None
//
//  Global Inputs:
//
//  Returns:            Bool    True  - signature is suppressed
//                              False - signature is allowed
//
//  Notes:
//
//-----------------------------------------------------------------------------
...
// puanlý iþlemde tüm puan harcanmýþ mý 
extern Bool TestTRN_AllPoint( void )
{
    if ( TRINP.TRLOYALTYSTATUS & TS_ALLPOINT )
        return True;
    
    return False;
}

// iþlem loyalty iþlem mi 
extern Bool TestTRN_Loyalty( void ) 
{ 
    if ( TRINP.TRLOYALTYSTATUS & TS_DISCOUNT )
        return True;
...
// Puanlý bir kartsa onus karttýr 
extern Bool TestTRN_OnusCard( void )
{
   if ( TRINP.TRFLAG & TR_HASPOINT ) 
      return True;

   if ( (TRREQ.TRKEY == VOIDTRAN) && (TRREQ.TRFLAG & TR_HASPOINT) )
   {
      memcpy( TRINP.AVAIL_LOYALTY_AMT, TRREQ.AVAIL_LOYALTY_AMT, S_TRTOTAM);
      return True;
   }   
   return False;
}
...
extern Bool TestTRN_UsedPin( void )
{
  if ( TRINP.TRSTATUS[1] & TS2_PINBLOCK )
    return True;
  else
    return False;
...
// müþteri kopya slibi bas True ise basar 
// false ise basmaz 
extern Bool TestFlagSecondCopy( void )
{
...
// Pan Trunc edilecek mi 
// True ise edilecek , False ise edilmeyecek 
extern Bool TestFlagPanTrunc( void )
{
  if ( TCONF.TCOPT6 & TC6_TRUNCPAN ) 
    return True;
   
  return False;
}

// sliplerde saat bilgisi çýkacak mý 
// true ise çýkacak false ise çýkmayacak 
extern Bool TestFlagPrintTime( void )
{
    if ( TCONF.TCOPT1 & T1_PRINTTIME ) 
      return True;
...
//-----------------------------------------------------------------------------
//  GetToken        Get the next PCL token, report error if no more tokens
//
//  Parameters:     None
//
//  Global Inputs:
...
//-----------------------------------------------------------------------------
//  PrintSubLine    Buffer the sub-line for printing
//
//  Parameters:     None
//
//  Global Inputs:
...
static Bool PrintSubLine( void )
{
	...
	switch ( PRJUST )
	{
		case LJ_LEFT:			// Left Justify Text on Line
			...
			if ( LineLoc + TrueLength > MaxCols )
			{
				if ( PRFLAG & PF_DBLE )	// Double wide printing?
				{
					PRBUF3LN = ( MaxCols - LineLoc ) / 2;
					TrueLength = PRBUF3LN << 1;
				}
				else
				{				// Single wide printing
					PRBUF3LN = MaxCols - LineLoc;
					TrueLength = PRBUF3LN;
				}
			}
			break;
		...
		case LJ_CENTER:		// Center Justify Text on Line
			...
			if ( MaxCols >= (TrueLength - 1) )
			{
				// Calculate current position within print line
				LineLoc = ( MaxCols - TrueLength + 1 ) >> 1;
			}
			else
			{
				LineLoc = 0;
...
//-----------------------------------------------------------------------------
//  PrintLineBuff   Print the current line of data with the handling
//                  of both Normal and Double Wide characters
//
//  Parameters:     Bool ForceIt        flush print buffer flag
//
//  Global Inputs:
//
//  Returns:        Bool        True  - okay, 
//                              False - error detected.
//
//  Notes:          Uses BUF1 and BUF2 to build printer data.
//
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
//  TST_FLD         Test to see if the field data exists
//
//  Parameters:     None
//
//  Global Inputs:
...
//-----------------------------------------------------------------------------
//   FINDFLD        Find the Field Definition in one of the field tables.
//                  Some of the fields from 160 to 230 are reserved, so
//                  that range will be skipped by the tables.
//
//  Parameters:     enum FLDTAB_ENUM Field  the field to find
//
//  Global Inputs:
//
...
//-----------------------------------------------------------------------------
//  ADR_LEN         Get the true Field Definition, Address and
//                  Length of the Field
//
//  Parameters:     None
//
...
//-----------------------------------------------------------------------------
//  EDITFLD         Builds a Character representation of the Field
//
//  Parameters:     None
//
//  Global Inputs:
	...
	if ( ADR_LEN(  ) )
	{
		...
		switch ( pFieldDef->F_EDIT )	// Figure out how to edit
		{
			...
			case E_CHRTRNC:	// ASCII Field -- truncated to Prt width
				...
				if ( S_HSTPRDATA > MaxCols )
					FLD_LEN = MaxCols;	// Yes; set field length to print line width
				else
			...
			case E_AMT1:		// Amount Field -- Receipt Amounts
				...
				if ( TRLOC->TRRSPOPT & R_APPROVED )
				{
					...
					CvtAmtPrint( FLD_ADR, pFieldData );	// Convert the Amount
			...
			case E_AMT3:		// Amount Field -- Amount possibly Negative
				...
				else
				{
					...
					CvtAmtPrint( FLD_ADR, HOLDAMT );	// Convert the Amount
...
//-----------------------------------------------------------------------------
//  FMT_FLD         Reformats a field
//
//  Parameters:     UBYTE Format    reformat type.
//
//  Global Inputs:
//
//  Returns:        Bool        True  - okay, 
...
//-----------------------------------------------------------------------------
//  FMT_NUM         Number Formats
//
//  Parameters:     UBYTE Format        reformat type.None
//
//  Global Inputs:
//
//  Returns:        Bool        True  - okay, 
...
//-----------------------------------------------------------------------------
//  FMT_STRLEN      Get the actual start and length of the field
//                  (excluding spaces)
//
//  Parameters:     UBYTE *pLength  pointer to actual length of the field 
//                                  (excluding spaces).
//
//  Global Inputs:
//
//  Returns:        char *None      Pointer to the actual start of the field..
//                                  (excluding spaces).
//
//  Notes:
//
...
//-----------------------------------------------------------------------------
//  GetName         Format the card holder's name for printing
//
//  Parameters:     None
//
//  Global Inputs:
...
//  Returns:        None
//
//  Notes:          TRNAME field is filled  in ACCTPROC().
//
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
//  SetTip          Process tip.  For non roll, if not 2 digits print **.
//
//  Parameters:     None
//
//  Global Inputs:
...
//-----------------------------------------------------------------------------
//  SetDefaultTxtTab        SetDefaultTxtTab
//
//  Parameters:
//      None
//  Global Inputs:
...
//-----------------------------------------------------------------------------
//  SetTxtTableType     SetTxtTableType
//
//  Parameters:
//      None
//  Global Inputs:
...
/*------------------------------------------------------------------
 *  Name:           PrintRemap
 *  Description:    Remap characters higher than 7Fh to internal
 *                  printer code page
 *
 *  Parameters:	    DstPtr  - destination pointer 
 					SrcPtr  - contains the data for remaping
 *					DataLen - is the size of the data
 *  Return Value:   None.
 *  Notes:          None.
 *-----------------------------------------------------------------*/
static void PrintRemap( void * pDest, const void * pSource, UWORD  Bytes )
...
//-----------------------------------------------------------------------------
//	PrintFirware	Description Line 1
//					Description line 2
//
