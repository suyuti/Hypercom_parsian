...
#define TCPO_TRANINS              0x04    // Tüm satýþ iþlemleri taksitli 
#define TCPO_DEFINS               0x08    // Varsayýlan Taksit
...
#define TCP_LIMITTRYREVERSAL      0x02    // reversal gönderme limiti 

struct termcapability_rec
{    
    UBYTE TCPOOP1;            // permissin options 1 
    //YTL support parametres
    UBYTE TCACTDT1[4];					
    UBYTE TCACTDT2[4];					
    UBYTE TCCURCD1[3];					
    UBYTE TCCURSIGN1[3];	
    UBYTE TCCURCD2[3];					    			
    UBYTE TCCURSIGN2[3];				
    UBYTE TCPDEFINS;    // varsayýlan taksit adedi
    UBYTE TCPMININS;    // minimum taksit adedi
    UBYTE TCPMAXINS;    // maksimum taksit adedi
    UBYTE TCPSALEMAXDIS[2]; // satiþ iþlemi için maksimum indirim oraný 
    UBYTE TCPINSMAXDIS[2];  // taksitli iþlemi için maksimum indirim oraný     
    UBYTE TCAPPEXPDT[3]; // APPLICATION EXPIRY DATE      
    UBYTE TCAPPDELDT[3]; // APPLICATION DELETION DATE   
    UBYTE TCPOPP2;      // options 
    UBYTE TCPREVTRYCOUNT; // reversal try count 
...
#define CR1_DEBITPRMT   0x01    // Debit Prompt @erd
#define CR1_DBTKEYONLY  0x02    // Debit Key Only @erd
#define CR1_UNUSED1      0x04    // Unused @erd
#define	CR1_2DIGISSN	0x08    // 2 DIGITS ISSUE NUMBER   
#define CR1_EBTKEYONLY  0x10	// Ebt Key Only @erd				
#define CR1_GIFTCARD	0x20    // Gift Card @erd
#define CR1_LYLTCARD	0x40	// Loyalty carc @erd
#define CR1_UNUSED2		0x80	// Unused @erd
...
//
#define ISPIN_SALE      0x01	// SALE PIN ENTRY
...
struct event_entry
{
	...
	char TRNTEXT[S_TRNTREXT];			// Transaction description
...
#define STYPE_LOAD              0x01
#define STYPE_INIT              0x02
...
#ifdef MAKE_SNMP
#define	IPCMAX	AQMAX*4 // TWO additional tables for SNMP
#else
...
struct ipcontab
{
	...
	UBYTE reserved[6];		// Reserved for feature use
} __attribute__((packed));
