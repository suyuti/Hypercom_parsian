...
/*-----------------------------------------------------------------------
 * Name:           RS232CFG
 * Description:    Configurate RS232 communication
 * Parameters:     None.
 * Return Value:   1 - Cancel or Clear pressed
...
//
	...
	do
	{
		...
		case '2': //  COMMS=COM2, PIN=COM1
            // This must be disabled for M2100 terminal
...
/*---------------------------------------------------------
 *  Name:           SetRSTabLRC
 *  Description:    Set the Checksum (LRC) for the RS232 Table
 *  Parameters:     None.
 *  Return Value:   None.
 *--------------------------------------------------------*/
...
//-----------------------------------------------------------------------------
//	TableInitRS232		Initialize RS232 table
//	Parameters:
//	Global Inputs:
//	Returns:
//	Notes:
...
//-----------------------------------------------------------------------------
//	Assign_COM1_COM2		Assign COM1 and COM2 ports for communication or PinPad
//	Parameters:             None
//	Global Inputs:          RSTAB
//	Returns:                None
//	Notes:
...
extern void Assign_COM1_COM2( void )
{
    ...
    // If COM1 or COM2 used for communication (not comm module)
    if ( RSTAB.RSOPT & RSOPT_RSACTIVE )
    {
        if ( RSTAB.RSOPT & RSOPT_COMNUM )
            // COM2 (RS232) selected for communication
            SDK_RS0SetDevice ( PIN ); 
        else
            // COM1 (RS422) selected for communication
            SDK_RS0SetDevice ( PRT );
    }


    // If COM1 or COM2 used for PinPad (comm module used for communication)
    else
    {
        if ( RSTAB.RSOPT & RSOPT_PINNUM )
            // COM2 (RS232) selected for PinPad
            SDK_RS0SetDevice ( PRT );
        else
            // COM1 (RS422) selected for PinPad
            SDK_RS0SetDevice ( PIN ); 
    }
}
