...
*/
    ...
    // boþ ise "0" yazmasý için gelen dataya '0' ekle
    if ( digitNumber == 0 ) 
    {
        digitNumber = 1;
        tmp[0] = '0';
    ...
    // Karakterleri üçer üçer ekrana bas
    while(digitNumber > 0) 
    {
...
// -------------------------------------------
//
// Karakter dizisini belirli uzunluk kadar saða kaydýrýr
// Parametre : 
...
extern void FormAmt( int cnt, char *pDest, const char *pBuf, char *CrrCode)
{
    ...
    // Formatlanmýþ tutarýn uzunluðunu bul
    totalLen = getFormattedLen( cnt, TCONF.TCDPP );
    // Para birimi kodunun uzunluðunu ekle
    totalLen += len;
    ...
    iSrc = cnt;
    /*
    // ONDALIK KISIM var mý?
    if ( TCONF.TCDPP > 0 )
    {
        // örn :0,000 için en soldaki 0 için +1
        len = TCONF.TCDPP + 1 ;
        //'0' ile doldur
        memset( &pDest[iDest-len], '0', len );
        // En soldaki '0' dan sonraki eleman ',' olmalý
        pDest[iDest-len+1] = ',';  

        // Ondalik kaç basamak girilmiþ
        ...
        // Ondalýk kýsmý kopyala
        memcpy( &pDest[iDest], &pBuf[iSrc], len);
        
        // Virgül var bir basamak daha geri gel
        --iDest ; 
    }else 
    {   
        // '0' kopyala
    ...
    */
    ...
    while(cnt > 0) 
    {
        ...
        // Karakterlerin tamamý yazýlmadý '.' koy
        if ( cnt > 0 ) 
        {
            --iDest;
            pDest[iDest] = '.';
...
/// <summary>
/// Formatsýz uzunlugu verilen tutarýn formatlý olarak uzunluðunu bulur
/// </summary>
/// <param name="len"> Tutar Uzunluðu</param>
/// <param name="decimalLen"></param>
/// <returns>int Formatlý Uzunluk</returns>
extern int getFormattedLen( int len, int decimalLen )
{
    // Formatlý halde kaç basamaklýk alan gerekir hesapla
    if ( decimalLen > 0 )
    {
        // Ondalýk kýsma 3 lü basamaklar için '.' konulmayacaðý için 
        // TCONF.TCDPP kýsmýný düþ
        len += ((len - decimalLen - 1) / 3 ); 
        // Ondalik kisim ile tam kýsmý ayýran ',' için
        ++len; 

        // Decimal alan (virgülle beraber) + virgülün solundaki 1 tane '0' olmalý
        if ( len < decimalLen + 1)
            len = decimalLen + 1;
        
    }else
    {
        // Her 4. , 7. , 10. alanlar için yeni bir '.' nokta konulur
        len += ((len - 1) / 3 );  
