...
static const ENTRY_STR GetInvEntryForPre = {
	N_ClosedPreAuth,                            // Title
	EnterInvNum,                                  // Prompt
	EnterInvNum,                                  // Default value
};
...
//-----------------------------------------------------------------------------
//  GetTranInv      Get transaction invoice number.
//
//  Parameters:     None
//
//  Global Inputs:  True - OK
//                  False - cancelled
//
//  Returns:        None
//
...
extern Bool GetTranInv( void )
{
	...
	do
	{
		...
		// Set title for AlphaNumEntry()
                if ( flagPreCancel == 1 ) 
                  Retval = AlphaNumEntry(1, S_TRINV, 1, &GetInvEntryForPre);
                else 
