...
extern void Fun_78( void )
{
	...
	if ( InputPswd( MsgId ) )
	{
		...
		do
		{
			...
			switch ( GetOneKey
					 ( DEV_KBD | DEV_PAD | STAY_OPEN0, UserTimeout ) )
			{
				case 'A':
				...
				case 'C':
					...
					if ( ENTER_KY == YesNoEntry( &YNEntry ) )
					{
						...
						CASHFMT = 0x55;
						// Clear the Table
						memset( ( UBYTE * ) & CASHTAB, 0,
							   CASHIER_SZE * CASHIER_MAX );
						// Reset the LRC
						CASHLRC = 0x00;
...
//-----------------------------------------------------------------------------
//  CashierStatus       Sign on/off a cashier according to its status
//
//  Parameters:         None
//
//  Global Inputs:
//
//  Returns:            UBYTE   0 - Cancel key pressed or timed-out
...
//-----------------------------------------------------------------------------
//  Signoff_All_Cashiers        Sign off all cashiers in the CASHTAB
//
//  Parameters:                 None
//
//  Global Inputs:
...
//-----------------------------------------------------------------------------
//  Cashier_View    Review cashier signed-on list
//
//  Parameters:     None
//
//  Global Inputs:
...
//-----------------------------------------------------------------------------
//  IsCashierOn     Find the cashier id contains in DspBuf from CASHTAB table
//
//  Parameters:     None
//
//  Global Inputs:
//
//  Returns:        BYTE    -1=Not found, or Index into CASHTAB
//
//  Notes:          The Cashier Id to look for is in DspBuf
//
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
//  GetCashier      Get user input of cashier number
//
//  Parameters:     Bool AllowNull      True  - allow NULL entry
//                                      False - do not allow NULL entry.
//
//  Global Inputs:
//
//  Returns:        BYTE    =0 - Valid cashier number was entered and index
