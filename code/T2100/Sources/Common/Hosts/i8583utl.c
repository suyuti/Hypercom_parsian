...
static void NMSUpdate( void );
static void ClrNMSStats( void );
...
//-----------------------------------------------------------------------------
//  Xfer            Transfer Offlines/Adjusts to Host
//
//  Parameters:     None
//
//  Global Inputs:
	...
	if ( HostEvent.TRNOPT1 & T1_PREADV )
	{
		...
		while ( True )
		{
			...
			else
			{
                ...
                {
                    // Specific comms module
		    if ( TM3_BLUETOOTH == (TERM.TERMOpt3 & TM3_HWTYPE_MASK) )
                        module =  TERM.TERMRemoteModule & TMR_HWTYPE_MASK;
		    else
                        module =  TERM.TERMOpt3 & TM3_HWTYPE_MASK;
                    switch ( module )
                    {
                        // V22 phone modem
                        case TM3_V22:
                        ...
                        case TM3_GSM_OLD:
                        case TM3_DIALBACKUP:
                            // Check if different phone number or modem mode 
                            // need to be used lather
                            if ( ( ( TRREQ.TRAQPTR->conn.dial.aqModem ) != TRREQ.TRAQPTR->AQSMODEM  ) ||
                              ( memcmp(&TRREQ.TRAQPTR->conn.dial.trans.primary.phoneNum, &TRREQ.TRAQPTR->conn.dial.settle.primary.phoneNum, S_AQPTTEL)))
                            {
                                // Yes, hung up now
                                Hang_Up ( );
                        ...
                        case TM3_ETHERNET:
                            // Check if different IP address or TCP port 
                            // need to be used lather

                            // Get IP settings pointers
                            pIPC1 = FindIPCById(TRREQ.TRAQPTR->AQ_TRANS_P_IPTAB);
                            pIPC2 = FindIPCById(TRREQ.TRAQPTR->AQ_SETTLE_P_IPTAB);
                            
                            if ( (NULL == pIPC1) || (NULL == pIPC1) )
                              break;

                            // Compare Tran and Settle IP settings
                            if(( memcmp(pIPC1->ipaddr, pIPC2->ipaddr, 15)) ||
                               ( memcmp(pIPC1->port, pIPC2->port, 5)) )

                            {
                                // Yes, hung up now
                                Hang_Up ( );

                                // Wait for hold time delay
...
//-----------------------------------------------------------------------------
//  Trickle         Trickle feed an Advice after a Transaction
//
//  Parameters:     None
//
//  Global Inputs:
...
//-----------------------------------------------------------------------------
//  SendAdvice      Send the next Advice to the Host
//
//  Parameters:     None
//
//  Global Inputs:
...
//-----------------------------------------------------------------------------
//  SetAdjustType   Set Adjust Type of the Transaction
//
//  Parameters:     None
//
//  Global Inputs:
...
//-----------------------------------------------------------------------------
//  Upload          Upload batch records to the host.
//
//  Parameters:     None
//
//  Global Inputs:
...
//-----------------------------------------------------------------------------
//  AllVoids        All voids in batch so do not settle.
//
//  Parameters:     None
//
//  Global Inputs:
...
// @ydeniz -- BC -- settlement arkasýndan istatistik yollanmasý
extern void SendStatistics( void )
{
	...
	// Move host response text.
	memcpy( TRINP.TRHOSTTEXT, TRREQ.TRHOSTTEXT, S_TRHOSTTEXT );
	
}

static void NMSUpdate( void )
{
	// Get EPROM program name
	memcpy( &NMSDAT.NMSwName, &VERSION, sizeof(NMSDAT.NMSwName) + sizeof(NMSDAT.NMSwRev));

	// Status is online
	NMSDAT.NMStatus = 0;

	// Terminal serial number
	memcpy( &NMSDAT.NMTSer, (UBYTE *)&TERM.TERMSer, sizeof(NMSDAT.NMTSer) );
	...
	// Convert SE PIN to BCD PIN in NMSDAT
	AscHex( (UBYTE *)NMSDAT.NMSepIn, ( char * ) TERM.TERMPwd, sizeof(NMSDAT.NMSepIn) );
}

//-----------------------------------------------------------------------------
//  ClrNMSStats			Clear and format NMSDAT.
//
//  Parameters:         None
//
//  Global Inputs:      None
...
static void ClrNMSStats( void )
{
	UBYTE DEFRTM[]={ 0x44, 0x00, 0x80, 0x01, 0x60, 0x04, 0x50, 0x09, 0x00};

	// Clear NMSDAT
	memset( (UBYTE *) &NMSDAT, 0x00, sizeof(NMSDAT));

	memcpy( (UBYTE *)&NMSDAT.NMRspCnt, &DEFRTM, sizeof(DEFRTM));
