...
extern UBYTE *pSendBuf;			// Pointer into transmit buffer 
#endif // MAKE_SMARTINIT


#ifdef	MAKE_EMV
extern void InitEMVTabs1( void );       // init EM4 page tables
extern void InitADTTab( void );         // init Additional tags table
extern void P60_LDIR_EMVTabs1( void );  // function for EM4 page downloads
extern void P60_LDIR_ADTTab( void );    // Additional Tags Table move
extern void CAPKTabPre( void );         // public keys table pre
extern void CAPKTabPost( void );        // public keys table post
extern void REVKTabPre( void );         // Key Revocation Table pre
extern void REVKTabPost( void );        // Key Revocation Table post
extern void ADTTabPre( void );          // Additional Tags Table pre
extern void ADTTabPost( void );         // Additional Tags Table post
...
//=============================================================================
#ifdef MAKE_SMARTINIT
UBYTE CRIndex;				// CRTAB index to support 254 card ranges
#endif // MAKE_SMARTINIT
...
struct subfield_hdr
{
	UBYTE sfTableId;			// Subfield table id 
	UWORD sfLength;				// Subfield length   
} __attribute__((packed));

// Initialize a Table record structure
struct table_rec
{
	UBYTE p60F_Type;                // Table Type Code                      
	void ( *p60F_Pre ) ( void );	// Prepare a Buffer to load             
	void ( *p60F_Mov ) ( void );	// Move a portion of the buffer         
	void ( *p60F_Post ) ( void );	// Set Checksum & other post_processing 
#ifdef MAKE_SMARTINIT
    UBYTE *pHASH;                   // Smart Init MD5 hash pointer
#endif // MAKE_SMARTINIT
} __attribute__((packed));
...
//=============================================================================
static UBYTE *pSFRec;				// Subfield pointer                   
static struct subfield_hdr *pSFHdr;	// Subfield header pointer            
static UBYTE *pSFDataSave;			// Temp save sub field pointer for rpt ld. 
static UWORD B60FBytesLeft;			// Bytes left in field                
//#ifndef MAKE_CERTIFICATE
static UWORD B60SBytesLeft;			// Bytes left in subfield             
//#endif // MAKE_CERTIFICATE
static UBYTE B60FIndex;				// Index in processing function table 
static UWORD MoveReportSize;		// Adjusted size for first report block 
static int IpIndex;					// Index to icepac file               
...
static Bool ExceedMaxEntryNum;		// Entry # exceeds maximum entry #
...
//-----------------------------------------------------------------------------
//  RspInit         Handle Response to Initialization.
//
//  Parameters:     None.
//
//  Global Inputs:  None.
...
//  Returns:        Nothing.
//
//  Notes:          Process Bit 60 in the Response Message
//
//                  For 0810 Responses, this is Initialization Data
//
//                  Type    0   Clear All Tables
//                          1   Terminal Configuration
//                          2 - Card Range Table Entry
//                          3 - Issuer Table Entry
//                          4 - Acquirer Table Entry
//                          5 - Descriptor Table Entry
//
//                  The message will look like:
//
//                      0LLL    Total Field Length
//
//                  Then one or more of...
//
//                      N2      Type of Table (1 byte)
//                      0LLL    Length of this Table (2 bytes)
//                      ....    Data
//
//                  If the "length of this table" is greater than the
//                  "length of this field" then the field data will be
//                  continued in the next message immediately following
//                  the "total field length" portion of the message.
//                  These continuation messages are repeated until the
//                  table is filled, at which time any additional bytes
//                  in the message start a new field.
//
//-----------------------------------------------------------------------------
...
static void ProcB60( void )
{
	...
	pSFRec = pResp;
	pSFHdr = ( struct subfield_hdr * ) pResp;
...
//-----------------------------------------------------------------------------
//  All_Clear  Clear all Initialization Tables.
//
//  Parameters:     None.
//
//  Global Inputs:  None.
...
//-----------------------------------------------------------------------------
//  Tconf_Pre   Terminal Configuration Table, Pre-Processing.
//
//  Parameters:     None.
//
//  Global Inputs:  None.
...
//-----------------------------------------------------------------------------
//  P60_0810_1Post  Terminal Configuration Table, Post-Processing.
//
//  Parameters:     None.
//
//  Global Inputs:  None.
...
//-----------------------------------------------------------------------------
//  Tcap_Pre   Terminal Capabilities Table, Pre-Processing.
//
//  Parameters:     None.
//
//  Global Inputs:  None.
...
{
	// Destination buffer 
	pB60DAddress = ( UBYTE * ) & TCAP;

	// How much to copy 
	B60DBytesLeft = TCAPSZE;
}


//-----------------------------------------------------------------------------
//  P60_0810_IdxPreL Indexed Table Pre-Processing.
//
//  Parameters:     UBYTE * pdest   Pointer to Array of Destination Buffers,
//                  UWORD szeRec    Size of one Dest Buffer in the array,
//                  UBYTE numRec    Number of Dest Buffers in the array,
//
//  Global Inputs:
//
...
//-----------------------------------------------------------------------------
//  DsTab_Pre   Descriptor Table, Pre-Processing.
//
//  Parameters:     None.
//
//  Global Inputs:  None.
...
//-----------------------------------------------------------------------------
//  New_SubField    Extract data from subfield header and set up to 
//                  process new subfield
//
//  Parameters:     None.
//
//  Global Inputs:  None.
...
//-----------------------------------------------------------------------------
//  Move_Some       Move some data from the subfield in the message
...
//-----------------------------------------------------------------------------
//  P60_LDIR        Move Data to a Local Buffer.
//
//  Parameters:     None.
//
//  Global Inputs:  None.
...
//-----------------------------------------------------------------------------
//  ApTab_Pre   Additional prompt Table, Pre-Processing.
//
//  Parameters:     None.
//
//  Global Inputs:  None.
...
//-----------------------------------------------------------------------------
//  V2Tab_Pre   Visa2 Table, Pre-Processing.
//
//  Parameters:     None.
//
//  Global Inputs:  None.
...
//-----------------------------------------------------------------------------
//  IbTab_Pre   Idle Screen Button Table, Pre-Processing.
//                  Description line 2
//
//  Parameters:     None.
//
...
//-----------------------------------------------------------------------------
//  BPre_LDIR       ICE Idle Screen Button Table, Processing.
//                  
//
//  Parameters:     None.
//
...
//-----------------------------------------------------------------------------
//  BPre_LRC        ICE Idle Screen Button Table, Post-Processing.
//                  
//
//  Parameters:     None.
//
...
//-----------------------------------------------------------------------------
//  IcePak_Pre   ICEPAK Configuration Table,-Processing.
//
//  Parameters:     None.
//
//  Global Inputs:  None.
...
//-----------------------------------------------------------------------------
//  GphAdd_Pre      ICEPAK Graphics Downloading.
//
//  Parameters:     None.
//
//  Global Inputs:  None.
...
//-----------------------------------------------------------------------------
//  GphAdd_LDIR     ICEPAK Graphics Downloading.
//
//  Parameters:     None.
//
//  Global Inputs:  None.
...
//-----------------------------------------------------------------------------
//  GphAdd_Post     ICEPAK Graphics Downloading.
//
//  Parameters:     None.
//
//  Global Inputs:  None.
...
//-----------------------------------------------------------------------------
//  FntAdd_Pre     Printer font downloading.
//  Parameters:     None.
//  Global Inputs:  None.
//  Returns:        Nothing.
//  Notes:          None.
...
//-----------------------------------------------------------------------------
//  FontAdd_LDIR    Printer font downloading.
//  Parameters:     None.
//  Global Inputs:  None.
//  Returns:        Nothing.
//  Notes:          None.
...
//-----------------------------------------------------------------------------
//  GphAdd_Post     Printer font downloading.
//  Parameters:     None.
//  Global Inputs:  None.
//  Returns:        Nothing.
//  Notes:          None.
...
//-----------------------------------------------------------------------------
//  RptAdd_LDIR     Report downloading.
//
//  Parameters:     None.
//
//  Global Inputs:  None.
...
//-----------------------------------------------------------------------------
//  RptAdd_Post     Report downloading.
//
//  Parameters:     None.
//
//  Global Inputs:  None.
...
//-----------------------------------------------------------------------------
//  FtTab_Pre  Footer Table Configuration Table,-Processing
//
//  Parameters:     None.
//
//  Global Inputs:  None.
...
/*------------------------------------------------------------------
 *  Name:           PDNTab_Pre
 *  Description:    Pre PDN table processing
 *  Parameters:     None.
 *  Return Value:   None.
 *  Notes:          
 *-----------------------------------------------------------------*/
...
/*------------------------------------------------------------------
 *  Name:           PDNTab_LDIR
 *  Description:    PDN table data move
 *  Parameters:     None.
 *  Return Value:   None.
 *-----------------------------------------------------------------*/
static void PDNTab_LDIR( void )
...
/*------------------------------------------------------------------
 *  Name:           AqTab_Pre
 *  Description:    Pre Acquirer table processing
 *  Parameters:     None.
 *  Return Value:   None.
 *  Notes:          
 *-----------------------------------------------------------------*/
...
/*------------------------------------------------------------------
 *  Name:           AqTab_LDIR
 *  Description:    Acquirer table data move
 *  Parameters:     None.
 *  Return Value:   None.
 *-----------------------------------------------------------------*/
static void AqTab_LDIR( void )
...
/*------------------------------------------------------------------
 *  Name:           CRTab_Pre
 *  Description:    Pre Card Range table processing
 *  Parameters:     None.
 *  Return Value:   None.
 *  Notes:          
 *-----------------------------------------------------------------*/
...
/*------------------------------------------------------------------
 *  Name:           CRTab_LDIR
 *  Description:    Card Range table data move
 *  Parameters:     None.
 *  Return Value:   None.
 *-----------------------------------------------------------------*/
static void CRTab_LDIR( void )
...
/*------------------------------------------------------------------
 *  Name:           IsTab_Pre
 *  Description:    Pre Issuer table processing
 *  Parameters:     None.
 *  Return Value:   None.
 *  Notes:          
 *-----------------------------------------------------------------*/
...
/*------------------------------------------------------------------
 *  Name:           IsTab_LDIR
 *  Description:    Issuer table data move
 *  Parameters:     None.
 *  Return Value:   None.
 *-----------------------------------------------------------------*/
static void IsTab_LDIR( void )
...
#ifdef MAKE_EMV
//-----------------------------------------------------------------------------
//  P60_LDIR_1      Move Data to a Buffer on EMV page
//  Parameters:     None
//  Global Inputs:
//  Returns:        None
//  Notes:
...
//-----------------------------------------------------------------------------
//  P60_LDIR_2      Move Data to a Buffer on EMV page
//
//  Parameters:     None
//
//  Global Inputs:
...
//-----------------------------------------------------------------------------
//  P60_LDIR_3      Move ADT Table Data to a Buffer on EMV page
//  Parameters:     None
//  Global Inputs:
//  Returns:        None
//  Notes:
...
//-----------------------------------------------------------------------------
//  IccTab_Pre   ICC Table, Pre-Processing
//
//  Parameters:     None
//
//  Global Inputs:
...
//-----------------------------------------------------------------------------
//  P60_0810_CPost  ICC Table, Post-Processing
//
//  Parameters:     None
//
//  Global Inputs:
...
//-----------------------------------------------------------------------------
//  TaidTab_Pre   TAID Table, Pre-Processing
//  Parameters:     None
//  Global Inputs:
//  Returns:        None
//  Notes:
...
//-----------------------------------------------------------------------------
//  P60_0810_DPost  TAID Table, Post-Processing
//  Parameters:     None
//  Global Inputs:
//  Returns:        None
//  Notes:
...
//-----------------------------------------------------------------------------
//  CapkTab_Pre   CAPK Table, Pre-Processing
//  Parameters:     None
//  Global Inputs:
//  Returns:        None
//  Notes:
...
//-----------------------------------------------------------------------------
//  P60_0810_EPost  CAPK Table, Post-Processing
//  Parameters:     None
//  Global Inputs:
//  Returns:        None
//  Notes:
...
//-----------------------------------------------------------------------------
//  P60_0810_FPost  REVK Table, Post-Processing
//  Parameters:     None
//  Global Inputs:
//  Returns:        None
//  Notes:
...
//-----------------------------------------------------------------------------
//  AtagTab_Pre  Additional Tags Table, Pre-Processing
//  Parameters:     None
//  Global Inputs:
//  Returns:        None
//  Notes:
...
//-----------------------------------------------------------------------------
//  P60_0810_10Post Additional Tags Table, Post-Processing
//  Parameters:     None
//  Global Inputs:
//  Returns:        None
//  Notes:
...
//-----------------------------------------------------------------------------
//  Name:           KyexTab_Pre
//  Description:    Key Expiry Dates Table, Pre-Processing
//	Parameters:
//	Global Inputs:
//	Returns:
//	Notes:
...
//-----------------------------------------------------------------------------
//  FontInit        Update the printer font.
//
//  Parameters:     None.
//  Global Inputs:  None.
//  Returns:        Nothing.
...
//-----------------------------------------------------------------------------
//  IpcTab_Pre   IP Connect Table, Pre-Processing.
//
//  Parameters:     None.
//
//  Global Inputs:  None.
...
#ifdef MAKE_SMARTINIT
//-----------------------------------------------------------------------------
//  Build_I1_hash   Build field 63, table "I1"
//
//  Parameters:     None
//
//  Global Inputs:  pSendBuf  - pointer to the buffer where build it.
//
//  Returns:        The number of hashes added.
//
//  Notes:          pSendBuf will be updated
//
//-----------------------------------------------------------------------------
...
//  Returns:        Nothing.
//
//  Notes:          Process Bit 63 in the Response Message
//
//                  For 0810 Responses, this is Initialization Data
//
//-----------------------------------------------------------------------------
