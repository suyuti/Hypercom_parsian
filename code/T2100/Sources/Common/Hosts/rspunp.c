...
struct save_rec
{
	UBYTE savbitno;				// Bit number to save for
	UBYTE *savdest;				// Where to save data
	void ( *sav_it ) ( void );	// Save routine
};
...
static const struct save_rec Save_Tab[] = {
	{  2,	 TRRSP.TRPAN,		SavPan	},	// Primary Account Number
	{  3,	 TRRSP.TRPROC,		GenSav	},	// Processing Code
	{  4,	 TRRSP.TRTOTAM,		GenSav	},	// Amount, Transaction
	{ 11,	 TRRSP.TRSTAN,		GenSav	},	// Systems Trace Audit Number
	{ 12,	 TRRSP.TRTIME,		GenSav	},	// Time, Transaction
	{ 13,	 TRRSP.TRDATE,		SavDate	},	// Date, Transaction
	{ 14,	 TRRSP.TREXPD,		GenSav	},	// Date, Expiry
	{ 22,	 TRRSP.TRPOSE,		GenSav	},	// POS Entry Mode
	{ 24,	 TRRSP.TRNII,		GenSav	},	// Network International Identifier
	{ 25,	&TRRSP.TRPOCC,		GenSav	},	// POS Condition Code
	{ 37,	 TRRSP.TRRRN,		GenSav	},	// Retrieval Reference Number
	{ 38,	 TRRSP.TRAUTH,		GenSav	},	// Auth Id Response code
	{ 39,	 TRRSP.TRRSPC,		GenSav	},	// Response Code
	{ 41,	 TRRSP.TRTID,		GenSav	},	// Terminal Id
	{ 42,	 TRRSP.TRACCID,		GenSav	},	// Acceptor Id
	{ 44,	 TRRSP.TRRSPA,		SavRSPA	},	// Additional Response Data
	{ 48,	 0,			SavKey	},	// PIN and MAC Keys
	{ 49,	 TRRSP.TRCODE,		SavCurCode },	// @@AA 15-03-06
	{ 54,	 0,	            	SavTIP	},	// Tip Amount
#ifdef MAKE_EMV
	{ 55,	 0,					SavICC  },  // ICC Data
#endif
	{ 60,	( UBYTE * ) &TRRSP,	SavB60	},	// Private Field 60
	{ 61,	( UBYTE * ) &TRRSP,	SavB61	},	// Private Field 61
	{ 62,	( UBYTE * ) &TRRSP,	SavB62	},	// Private Field 62
	{ 63,	( UBYTE * ) &TRRSP,	SavB63	},	// Private Field 63
#ifdef MAKE_ISOMAC
	{ 64,	( UBYTE * ) &TRRSP,	SavB64	},	// Private Field 64, MAC
#endif // MAKE_ISOMAC
	{  0,	( UBYTE * ) &TRRSP,	GenSav	}	// Table End
};
...
static void SavCurCode( void )
{
	...
	// Move the data
	memcpy( TRRSP.TRCODE, pData, 3 );
}


//-----------------------------------------------------------------------------
//  RspUnPackMsg    Unpack the Message Response into TRRSP.
//
//  Parameters:     None.
//
//  Global Inputs:  None.
...
//-----------------------------------------------------------------------------
//  SavData         Save Data from Response Data.
//
...
//-----------------------------------------------------------------------------
//  FindSaveEntry   Find Save Table Entry.
//
//  Parameters:     UBYTE bitno     bit number.
//
//  Global Inputs:  None.
//
//  Returns:        struct save_rec *   Pointer to entry for bit number.
//
//  Notes:          None.
//
//-----------------------------------------------------------------------------
...
static struct save_rec *FindSaveEntry( UBYTE bitno )
{
	...
	while ( ( pSaveRec->savbitno != 00 ) && ( pSaveRec->savbitno != bitno ) )
	{
		// Look at next record
		pSaveRec++;
	}
...
//-----------------------------------------------------------------------------
//  GenSav          General Purpose Save Routine.
//
//  Parameters:     None.
//
//  Global Inputs:  None.
...
//-----------------------------------------------------------------------------
//  SavPan          Save Primary Account Number Field.
//
//  Parameters:     None.
//
//  Global Inputs:  None.
...
//-----------------------------------------------------------------------------
//  SavTIP          Save the Tip from the Response Message.
//
//  Parameters:     None.
//
//  Global Inputs:  None.
...
//-----------------------------------------------------------------------------
//  SavDate         Save Date from Response
//
//  Parameters:     None.
//
//  Global Inputs:  None.
...
//-----------------------------------------------------------------------------
//  SavRSPA         Save Additional Response Data.
//
//  Parameters:     None.
//
//  Global Inputs:  None.
...
//-----------------------------------------------------------------------------
//  BumpPtr         Bump Response Pointer along, according to IS8583.
//
//  Parameters:     None.
//
//  Global Inputs:  None.
...
#ifdef MAKE_EMV
//-----------------------------------------------------------------------------
//  SavICC          Save ICC data from response
//
//  Parameters:     None
//
//  Global Inputs:
...
//-----------------------------------------------------------------------------
//  Field55Parse    Parse Field 55 (ICC System Information Data)
//
//  Parameters:     UBYTE *fldptr   pointer to start of Field 55
//
//  Global Inputs:
//
//  Returns:        None
//
//  Notes:          If there's an error in Field 55 structure - 
//                  the whole field is ignored.
//                  Uses globals IssScriptBuff & ScriptLen from appdata.c
//-----------------------------------------------------------------------------
static void Field55Parse( UBYTE * fldptr )
{
	...
	if ( pTmp != NULL &&
		 !( TRREQ.TRSTATUS[0] & ( TS1_REV + TS1_ADV + TS1_VOID ) ) )
	{
