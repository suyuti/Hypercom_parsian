...
Bool DelimFlag;			// Print Hex string with spaces if True, w/out if False
...
static const char MandatoryCRMNotPerformed[] = "Mandatory CRM not performed\n";

static const char IASupportNotPerformed[] = "IA supported but not performed\n";
static const char IAPerformedNotSupport[] = "IA performed when not supported\n";


// Converts Hex to ascii and inserts spaces if necessary, null terminates the line
void Hex2String( UBYTE * ICCtag, UBYTE length, char *buffer )
{
	static UBYTE *pSrc;
	static char *pDest;
...
// Convert binary value to BCD (lSrc <= 4) 
void Bin2Bcd( UBYTE* pDest, UBYTE* pSrc, UBYTE lDest, UBYTE lSrc )
{
	int i;
	unsigned long src = 0;
...
/*-----------------------------------------------------------------------
 * Name:           PrintICCDebug
 * Description:    Print ICC debug data
 *
 * Parameters:     None
 * Return Value:   None
 * Notes:          
...
void PrintICCDebug( UBYTE emv_mode, CandTextType * PrintCand, UBYTE *PrevAmount )
{
	...
	if ((IccDebFilePtr = fopen( IccDebFileName, "wb" )) == NULL) return;
	{
		...
		{
			memcpy( PrtBuffer, "DA ", 3 );
			memcpy( &PrtBuffer[3], PerformedNotRequested, sizeof(PerformedNotRequested) );
			PrintBuf( );
		}
		...
		{
			memcpy( PrtBuffer, "TRM ", 4 );
			memcpy( &PrtBuffer[4], RequestedNotPerformed, sizeof(RequestedNotPerformed) );
			PrintBuf( );
		}
		else if ( !( aip & 0x08 ) && ( tsi & 0x08 ) )
		{
			memcpy( PrtBuffer, "TRM ", 4 );
			memcpy( &PrtBuffer[4], PerformedNotRequested, sizeof(PerformedNotRequested) );
			PrintBuf( );
		}
...
// Added IssScriptBuf & ScriptLen content printing
				...
				PrintBuf( );
			   	...
			   	switch ( ICCtag[0] )
			   	{
					...
					default :
					{
						...
						itoa( MaxPerCent, &PrtBuffer[12],10 );
						PrintBuf( );
						memcpy( PrtBuffer, "   TargPerc=", 12 );
						itoa( TargetPerCent, &PrtBuffer[12],10 );
						PrintBuf( );
						...
						if ( RandNum > TargetPerCent )
						{
							...
							if ( memcmp( TxnAmount, Threshold, 6 ) >= 0 )
							{
								...
								PrtBuffer[26] = 'X';
								PrtBuffer[28] = '(';
								itoa( MaxPerCent, &PrtBuffer[29],10 );
								PrtBuffer[31] = '-';
								itoa( TargetPerCent, &PrtBuffer[32],10 );
								PrtBuffer[34] = ')';
								itoa( TargetPerCent, &PrtBuffer[38],10 );
								PrtBuffer[36] = '+';
								PrintBuf( );
        ...
        // write auto test data to the card				
		SC_CmdLen = Cmd + SC_CmdBytes[Lc]; // set length
        ScrdSendCmd(scrd_SCR, True, SC_CmdBytes, SC_CmdLen, SC_RspBytes,	
                    &SC_RspLen, SC_SW12, &SC_CmdStatus, &SC_NAD);
	}
...
#ifdef	False
/*------------------------------------------------------------------
 *  Name:           PrintPinpadStatus
 *  Description:    Print External Pinpad info
 *  Parameters:     none
 *  Return Value:   none
 *  Notes:		    Called by Function 61 (FUN_61)
 *-----------------------------------------------------------------*/
void PrintPinpadStatus( void )
{
    PrintLine ("================");
...
/*------------------------------------------------------------------
 *  Name:           PrintLine
 *  Description:    Print the null-terminated line
 *  Parameters:     char * pString
 *  Return Value:   none
 *  Notes:		    
 *-----------------------------------------------------------------*/
static void PrintLine( char* pString )
...
/*------------------------------------------------------------------
 *  Name:           PrintBuf
 *  Description:    Print the content of PrtBuffer
 *  Parameters:     none
 *  Return Value:   none
 *  Notes:		    
 *-----------------------------------------------------------------*/
	...
	if ((IccDebFilePtr = fopen( IccDebFileName, "rb" )) != NULL) 
	{
		...
		if ( orvOK == SDK_PrinterOpen( TCONF.TCPRINTER | ((TCONF.TCOPT2 & TC2_GRAPHMODE) ? 0x80 : 0x00) ))
		{
			while (1)
			{
				...
				if ( orvOK != SDK_PrintBuffer( bufferprn, buffercnt ) ) break;
...
//-----------------------------------------------------------------------------
//  PrintAppCap: Print App Cap settings
//  Parameters:     None
//  Global Inputs: None
//  Returns:        None
