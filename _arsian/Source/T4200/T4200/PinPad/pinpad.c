	...
	else
	{
		...
		else
		{
			...
			while( ! ( readflag & 0x01 ) )
			{
				if( bCheckCancel )
				{
					InputType = read_io( DEV_PAD + DEV_KBD + DEV_PINCARD, timeout, &bPinCardStatus, &DevFlag );	// Wait for input
					switch ( InputType )
					{
						case DEV_PAD:
                             ...
                             UBYTE *dest, UBYTE *retStatus )
{
    ...
    char promptline[17] = {0};
        ...
        *retStatus = 2;//EMVL2_PINPAD_FAILURE ;
        ...
        if ( orvOK == status )
        {
            ...
            if( PINBUF[0] == '3' && PINBUF[1] == '3' )
            {
                ...
                default:
                    {
                        // Cancelled by user
                        *retStatus = 60;//EMVL2_ONLINE_CANCELLED;
                        bRetVal = True;
                        break;
                    }
                }
            ...
            else
            {
                ...
                *retStatus = 4;//EMVL2_PINPAD_FAILURE;
                bRetVal = True;
            }
        }
        ...
        {
            // PIN entry bypassed
            *retStatus = 4;//EMVL2_PIN_BYPASSED;
            bRetVal = True;
        }
        else
        {
		...
		if ( orvOK == status )
		{
                ...
                */
          		  if (PINBUF[3] == '0')  // byte0 = 0
          		  {
                      switch (PINBUF[4]){
          				case '0':
          					*retStatus  = 0;//EMVL2_PIN_OK ;
          					break ;
          				case '1':
          				case '2':
          					*retStatus  = 5;//EMVL2_PINICC_FAIL ;
          					break ;
          				case '3':
          					*retStatus  = 4;//EMVL2_PIN_BYPASSED ;
          					break ;
          				...
          				case '5':
							...
							{
								if( PINBUF[6] == 0xC0 )
	          						*retStatus = 3;//EMVL2_PIN_BLOCKED ; // overwrite status
							}
              ...
              UBYTE *retStatus )
{
    ...
    while(1)
    {
        ...
        ev = VPPGetPin( 12, Dspbuf2, TRINP.TRPINBL, UpdateInputBox, sw1sw2 );
...
Bool VPP_OffLine( VPP_EMV_ActionType type, UBYTE* publicKey, UBYTE publicKeyLength, UBYTE tries, UBYTE* retStatus, UBYTE *sw1sw2 )
{
	*retStatus = EMVL2_PINPAD_FAILURE;
    ...
    do
    {
        ...
        switch( VPPGetPin( 12, Dspbuf2, TRINP.TRPINBL, UpdateInputBox, sw1sw2 ))
        {
			// Check if pin number has been entered OK
			case VPP_EnterKey:
		...
		if ( OK == DeviceRead (PIN_DEVICE, &PINBLKR, PROCEED+FLAG, PINBUF, sizeof(PINBUF), &eventflag, 7 ) )
		{
			...
			while( ! ( eventflag & 0x01 ) )
			{
				if ( chkkbd != 0)
				{
					...
					InputType = read_io( DEV_KBD + DEV_PAD + STAY_OPEN0 + DEV_PINCARD, timeout, &bPinCardStatus, &DevFlag );
					if ( InputType )
					{
