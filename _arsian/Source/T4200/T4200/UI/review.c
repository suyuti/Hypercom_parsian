...
extern void PadLeft( char *pBuffer, const char *pString, UBYTE Length, char FillChar )
{
    short index;
    char *str;
    ...
    if ((pBuffer) && (Length))
    {
        memset (pBuffer, FillChar, Length);
        pBuffer[Length] = '\0';
        for (index = 0; (index < Length) && (pString[index] != 0); index++);
        for (str = &pBuffer[Length - 1], --index; index >= 0; index--)
            *str-- = pString[index];
    }
}
...
extern void Find( void )
{
    ...
    while ( loopflag )
    {
        ...
        if ( 0 == ( AlphaNumEntry( 1, S_TRSTAN * 2, 1, &InvoiceNumEntry ) ) )
        {
        ...
        // Invoice number not used in Parsian Project
        // Copy invoice number that was input to search record
        // MovAscZFill( ( char * ) JSREC.JSINV, Dspbuf, S_TRINV, ' ' );

        // Copy STAN number that was input to search record
        PadLeft((char *) tempstr, (char *) Dspbuf, S_TRSTAN * 2, '0' );
        AscHex(( char * ) JSREC.JSSTAN, tempstr, S_TRSTAN);
        tempstr[2 * S_TRSTAN] = 0;
