...
//=============================================================================
//extern UWORD Verify_Idle_Btn( UWORD id );
...
//             |RSPOPT                    |RSPOPT2
	...
	{"14", "", 0,                         0 },
	{"19", "", 0,                         0 },
	{"25", "", R_REVAPPROVED,                0 }, // for Parsian Bank only!...
	{"30", "", 0,                         0 },
    {"31", "", 0,                         0 },
    {"34", "", R_REVAPPROVED,             0 },
	{"41", "", 0,                         0 },
	...
	if ( tempmsg >= MSG_SPECIAL || tempmsg <= CustomMsg8 )
	{
        ...
        case MSG_ICEBUTTON1_L1:
            //ptr = IBTAB.IBBTN1LABEL1;
                ...
                eventid = Verify_Idle_Btn(EVENT_ICEBUTTON1);
                if (eventid == NOT_SUPPORTED)
                    ptr = IBTAB.IBBTN1LABEL1;
                else
                {
                    MLGetMessage.tblid = evttext_table_id;
                    MLGetMessage.msgid = eventid;
			...
			case MSG_ICEBUTTON2_L1:
                ...
                eventid = Verify_Idle_Btn(EVENT_ICEBUTTON2);
                if (eventid == NOT_SUPPORTED)
                    ptr = IBTAB.IBBTN2LABEL1;
                else
                {
			...
			case MSG_ICEBUTTON3_L1:
                ...
                eventid = Verify_Idle_Btn(EVENT_ICEBUTTON3);
                if (eventid == NOT_SUPPORTED)
                    ptr = IBTAB.IBBTN3LABEL1;
                else
                {
            ...
            case MSG_ICEBUTTON4_L1:
                ...
                eventid = Verify_Idle_Btn(EVENT_ICEBUTTON4);
                if (eventid == NOT_SUPPORTED)
...
extern void GetRspTxtOpts( void )
{
    ...
    // Parsian'da Reversal için 25, 34, 02 kodlari APPROVED kabul edilir.
    //if (TRINP.TR) {
    //}
