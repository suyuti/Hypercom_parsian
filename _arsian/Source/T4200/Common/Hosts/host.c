...
//=============================================================================
//! Active Acquirer's ID
UBYTE CurrentAqid;
//! Temporary data storage for host functions
UBYTE TmpBuf[256];
//! Host entry pointer
HOSTENTRY *Host;
...
//!
//! \note
//!     This Procedure should loop forever!!!
//-----------------------------------------------------------------------------
extern void INITHOST( void )
{
	...
	do
	{
		...
		if( SDK_WaitForEvents( EVENT_QUIT | EVENT_HOSTQRY, &ev, SDK_SUSPEND ) )
		{
			ev&= ~EVENT_QUIT;
		}
		else if( ev & EVENT_HOSTQRY )
		{
			SDK_ClearEvents( EVENT_HOSTQRY );
