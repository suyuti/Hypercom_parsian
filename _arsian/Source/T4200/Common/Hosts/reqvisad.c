...
#define ATC_DEBIT	     "93"
//! DEBIT REFUND
#define ATC_DEBIT_REF    "94"
//! DEBIT BALANCE INQUIRY
#define ATC_DEBIT_BI     "9A"
//! CREDIT REFUND (OFFLINE)
#define ATC_REFUND	     "CR"
//! Device Code.
#define V2_DEVICE        "00"
//! 3 VISA II FIELDS: RECORD FORMAT, APPLICATION TYPE AND DELIMITER.
#define DBC_FORMAT      "V0."
//! 3 VISA II FIELDS: RECORD FORMAT, APPLICATION TYPE AND DELIMITER.
#define V2_FORMAT       "D0."
//! 3 VISA II FIELDS AS NOTED IN ABOVE COMMENT.
#define V2_DEBIT_FMT    "T0."
//! '00' = ENGLISH
#define V2_LANGUAGE_IND  "00"
//! GROUP III VERSION 001 - COMMERCIAL CARD REQUEST INDICATOR
#define V2_GROUP_III_001 "001!010"
//! GROUP III VERSION 007 - CVV2
#define V2_GROUP_III_007 "007"
//! DEVICE & INDUSTRY CODES - DIAL TERMINAL RETAIL
#define V2_DEVICE_RETL   "DR"
//! DEVICE & INDUSTRY CODES - DIAL TERM RESTAURANT
#define V2_DEVICE_REST   "DF"
//! THIS FIELD CONTAINS 4 VISA II
#define V2_HEAD        "K1.Z"
//! FIELDS: RECORD FORMAT, APPLICATION TYPE, MSG DELIMITER, & X.25 ROUTING ID
#define V2_RTYPE      " @@@@"
...
#define S_ATC_MO	       2
//! CARD AUTHENTICATION
#define S_ATC_CARDV	       2
//! DEBIT PURCHASE
#define S_ATC_DEBIT	       2
//! DEBIT REFUND
#define S_ATC_DEBIT_REF    2
//! DEBIT BALANCE INQUIRY
#define S_ATC_DEBIT_BI     2
//! CREDIT REFUND (OFFLINE)
#define S_ATC_REFUND	   2
//! Device Code.
#define S_V2_DEVICE        2
//! 3 VISA II FIELDS: RECORD FORMAT, APPLICATION TYPE AND DELIMITER.
#define S_DBC_FORMAT       3
//! 3 VISA II FIELDS: RECORD FORMAT, APPLICATION TYPE AND DELIMITER.
#define S_V2_FORMAT        3
//! 3 VISA II FIELDS AS NOTED IN ABOVE COMMENT.
#define S_V2_DEBIT_FMT     3
//! '00' = ENGLISH
#define S_V2_LANGUAGE_IND  2
//! GROUP III VERSION 001 - COMMERCIAL CARD REQUEST INDICATOR
#define S_V2_GROUP_III_001 7
//! GROUP III VERSION 007 - CVV2
#define S_V2_GROUP_III_007 3
//! DEVICE & INDUSTRY CODES - DIAL TERMINAL RETAIL
#define S_V2_DEVICE_RETL   2
//! DEVICE & INDUSTRY CODES - DIAL TERM RESTAURANT
#define S_V2_DEVICE_REST   2
//! THIS FIELD CONTAINS 4 VISA II
#define S_V2_HEAD          4
//! FIELDS: RECORD FORMAT, APPLICATION TYPE,
#define S_V2_RTYPE         5
...
//! Pointer to V2TAB entry  
static struct v2_rec *pVisa2Ptr;
//! Pointer into send buffer 
static UBYTE *pSendBuff;
//! Second Pointer into send buffer 
static UBYTE *pSendBuffSave;
//! HASHING TOTAL 
static UBYTE HashTotal[S_TRTOTAM];
//! CASH BACK TOTAL 
static UBYTE CashBackTotal[S_TRTOTAM];
//! NET DEPOSIT 
static UBYTE NetTotal[S_TRTOTAM];
//! RECORD COUNTER 
static UBYTE RCOUNT[S_RCOUNT];
...
//-----------------------------------------------------------------------------
//! \brief
//!     Moves Authorization Transaction Code into send Buffer
//!
//! \return
//!     None
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
//! \brief
//!     Moves Cardholder identification code into send buffer
//!
//! \return
//!     None
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
//! \brief
//!     Moves account data source code into send buffer
//!
//! \return
//!     None
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
//! \brief
//!     Moves customer data field into send buffer
//!
//! \return
//!     None
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
//! \brief
//!     Moves Cardholder identification DATA into send buffer
//!
//! \return
//!     None
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
//! \brief
//!     Moves the secoundary amount field, see notes bellow
//!
//! \return
//!     None
//!
//! \note
//!     The visa ii 'J'/'G' authorization message formats use the secondary
//!     amount field for cash back amounts on debit sales only. no tip amount
//!     is ever put in this field.
//-----------------------------------------------------------------------------
static void MoveSECAMT( void )
{
	static int i;
...
//-----------------------------------------------------------------------------
//! \brief
//!     Moves the VISA II SHARING GROUP data to send buff till '0' or ' '
//!
//! \return
//!     None
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
//! \brief
//!     Finds batch number for this acquirer and moves the last three digits
//!     of it into send buffer for settlement header and trailer messages
//!
//! \return
//!     None
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
//! \brief
//!     Moves reimbursement attribute in send buffer
//!
//! \return
//!     None
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
//! \brief
//!     Moves the debit aba no data to send buff if it is not all ' 's or
//!     '0's or 0s.
//!
//! \return
//!     None
...
//-----------------------------------------------------------------------------
//! \brief
//!     Moves the V2DEBIT_SETTLEAGENT data to send buff if it is not all ' 's
//!     or '0's or 0s.
//!
//! \return
//!     None
...
//-----------------------------------------------------------------------------
//! \brief
//!     Moves transaction amount in send buffer for sale/auth only transactions
//!
//! \return
//!     None
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
//! \brief
//!     Builds a visanet, base ii debit confirmation for all debit trans
//!
//! \return
//!     None
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
//! \brief
//!     Moves the first 5 fields into the record
//!
//! \return
//!     None
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
//! \brief
//!     Builds a visanet, base ii settlement batch trailer message is for 'K'
//!     device types, record type is 'T'
//!
//! \return
//!     None
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
//! \brief
//!     Builds a visanet, base ii settlement header message this is for 'K'
//!     device types, record type is 'K'
//!
//! \return
//!     None
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
//! \brief
//!     Builds a visanet, base ii settlement TERMINAL PARAMETER message this
//!     message is for the 'K' device type, record type is 'P'
//!
//! \return
//!     None
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
//! \brief
//!     Builds a visanet, base ii DETAIL TRANSACTION message for message is
//!     for 'K' device types, record type is 'D'
//!
//! \return
//!     None
//-----------------------------------------------------------------------------
...
//        BfAscii( ( char * ) pSendBuff, TRREQ.TRJALALIDATE, S_TRDATE );
//    }
//    else {
...
//    }
	...
	*pSendBuff = '0';
	...
	// Group 2 -- Restaurant 
	//IS RESTAURANT OPTION ENABLED ? 
	if ( TCONF.TCOPT1 & TC1_REST )
	{
		// Gratuity Amount 
		BfAscii( ( char * ) pSendBuff, TRREQ.TRTIP, S_TRTIP );

		// Update the data pointer 
		pSendBuff += S_TRTIP * 2;

		// Set 1 bit  
		*( pSendBuffSave + 4 ) |= BIT_1_SET;
	}
