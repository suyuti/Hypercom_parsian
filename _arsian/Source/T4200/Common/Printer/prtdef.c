...
const UBYTE ALIGN(4) RP_RECEIPT_DATA[] = {
    ...
    RGHT, TXT, TerminalID, 
    LFT, FLD_F, FLD_TID, FC_TRIM,
    LINE,

    RGHT, TXT, Merchant, 
    LFT, FLD_F, FLD_MID, FC_TRIM,
    LINE,

    // Transaction Name: Farsca ve Ingilizce olacak.
    IF, CND_ROLL, 
        DOUBLE, 
    ENDIF,
    ...
    IF, TRN_DBCD, OR, TRN_BALI,
        LFT, MLTCHR, 12, '.', FLD_S, FLD_PAN, ( UBYTE )-6,   // Always truncate PAN for these
    ELSE,
        IF, FLG_TCONF, TCOPT6, TC6_TRUNCPAN,    // PAN truncation enabled?
            IF, FLG_TCONF, TCOPT6, TC6_NOSECOND, OR, OPT5,
                ...
                LFT, MLTCHR, 12, '.', FLD_S, FLD_PAN, ( UBYTE )-6,   // Truncated PAN
            ELSE,
                LFT, FLD, FLD_PAN,               // Merchant copy of receipt
            ENDIF,
        ELSE,
            LFT, FLD, FLD_PAN,               // PAN truncation disabled
        ENDIF,
    ENDIF,
    IF, CND_MAN, ' ', '.', ENDIF,
    LINE,
    ...
    RGHT,TXT, CardHolderName,   // !!!card name alaný eklenecek
    LFT, FLD_F, FLD_NAME, FC_TRIM,
    LINE,
    ...
    // Stan
    RGHT, TXT, Stan,    //!!! Stan alaný eklenecek     
    LFT, FLD, FLD_STAN,
    LINE, 
...
// 
    RGHT, TXT, Time, 
    LFT, FLD_FL, FLD_TRNTM, FT_MIL, 5, ' ',
    LINE,

    RGHT, TXT, Date,
    LFT, FLD_F, FLD_TRNDT, FD_JEJALI_TEXT,
    LINE,
...
// Sale amount
    ...
    */
    ...
    //IF, NOT, CND_REJ,
        ...
        RGHT,
        TXT, Imza, 
        //' ',' ',' ',' ',' ',TXT, CardNumber, LINE,       // sign alaný eklenmeli buraya    
        LINE,
    //ELSE,
    //    TXT, TransactFailed,
    ENDIF,
...
//! Receipt printing
const UBYTE ALIGN(4) OLD_RP_RECEIPT_DATA[] = {
...
/*FARSI*/// Till number
/*FARSI*///*****************************************************************************
/*FARSI*/    IF, FLG_TCONF, TCOPT1, TC1_TILL,
/*FARSI*/        FLD_F, FLD_TILLNO, FC_TRIM,
/*FARSI*/        IF, CND_ROLL,
/*FARSI*/            TXT, Till,
/*FARSI*/        ELSE,
/*FARSI*/            ' ', TXT, Csh,
...
/*FARSI*/
/*FARSI*///*****************************************************************************
/*FARSI*/// Cashier (server) number
/*FARSI*///*****************************************************************************
/*FARSI*/    IF, FLG_TCONF, TCOPT1, TC1_CASHIER, AND, CND_ROLL,
/*FARSI*/        FLD_F, FLD_SRVID, FC_TRIM,
/*FARSI*/        IF, FLG_TCONF, TCOPT4, TC4_SRV_PRM,
/*FARSI*/            TXT, Srv,
/*FARSI*/        ELSE,
/*FARSI*/            TXT, Csh,
/*FARSI*/        ENDIF,
...
/*FARSI*/
/*FARSI*///*****************************************************************************
/*FARSI*/// Card (issuer) name
/*FARSI*///*****************************************************************************
/*FARSI*/    RGHT, FLD, FLD_CNAME,
/*FARSI*/    LINE,
/*FARSI*/
/*FARSI*/  ELSE, // LNG_FARSI %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
/*FARSI*/
/*FARSI*///*****************************************************************************
/*FARSI*/// Card expiration date
/*FARSI*///*****************************************************************************
/*FARSI*/    NORMAL,
/*FARSI*/    IF, FLD_EXPDT,
/*FARSI*/        IF, FLG_TCONF, TCOPT6, TC6_NOSECOND, OR, OPT5,
/*FARSI*/            // This is either 2-part paper or the customer copy.
/*FARSI*/            IF, FLG_TCONF, TCOPT6, TC6_TRUNCPAN,    // PAN truncation enabled?
/*FARSI*/                // Yes, don't show expiration date on customer receipt
/*FARSI*/            ELSE,
/*FARSI*/                // No, so show expiration date.
/*FARSI*/                FLD_F, FLD_EXPDT, FD_MDYS,
/*FARSI*/                IF, CND_ROLL, 
/*FARSI*/                    ' ', ' ', ':','E', 'X', 'P', '.',  
/*FARSI*/                ENDIF, 
/*FARSI*/            ENDIF,
/*FARSI*/        ELSE,
/*FARSI*/            // Show expiration date on the merchant receipt.
/*FARSI*/            FLD_F, FLD_EXPDT, FD_MDYS,
/*FARSI*/            IF, CND_ROLL, 
/*FARSI*/                ' ', ' ', ':','E', 'X', 'P', '.',  
/*FARSI*/            ENDIF, 
...
/*FARSI*/        TXT, Void_s,
/*FARSI*/    ELSE,
/*FARSI*/        FLD_F, FLD_TRNNAME, FC_TRIM,
/*FARSI*/    ENDIF,
/*FARSI*/    LINE,
/*FARSI*/
/*FARSI*/  ELSE, // LNG_FARSI %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
/*FARSI*/  IF, LNG_FARSI, //%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
/*FARSI*///*****************************************************************************
/*FARSI*/// Invoice Number
/*FARSI*///*****************************************************************************
    'S', 'T', 'A', 'N', ':',
/*FARSI*/    FLD, FLD_STAN, //FLD_INVNO,
/*FARSI*/    IF, CND_ROLL,
/*FARSI*/        ' ', TXT, Invoice,
/*FARSI*/    ENDIF, 
/*FARSI*///*****************************************************************************
...
/*FARSI*/
/*FARSI*///*****************************************************************************
/*FARSI*/// Transaction Date/Time
/*FARSI*///*****************************************************************************
/*FARSI*/    IF, FLG_TCONF, TCOPT1, TC1_PR_TIME,
/*FARSI*/        FLD_FL, FLD_TRNTM, FT_MIL, 5,' ',
/*FARSI*/        IF, CND_ROLL,
/*FARSI*/            TXT, Time,
/*FARSI*/        ENDIF,
/*FARSI*/    ENDIF,
/*FARSI*/    
/*FARSI*/    RGHT,
/*FARSI*/    FLD_F, FLD_TRNDT, FD_JEJALI_TEXT, //FD_TEXT,
/*FARSI*/    IF, CND_ROLL,
/*FARSI*/        ' ', TXT, Date,
/*FARSI*/    ENDIF, 
/*FARSI*/    LINE,
...
//*****************************************************************************
    IF, CND_ROLL,
        TXT, Date,
    ENDIF, 
    FLD_F, FLD_TRNDT, FD_JEJALI_TEXT, //FD_TEXT,
...
/*FARSI*/
/*FARSI*///*****************************************************************************
/*FARSI*/// Authorisation code
/*FARSI*///*****************************************************************************
/*FARSI*/        IF, FLD_AUTH,
/*FARSI*/            RGHT, 
/*FARSI*/            FLD, FLD_AUTH,
/*FARSI*/            IF, CND_ROLL, ' ', TXT, AuthN, ENDIF,
/*FARSI*/        ENDIF,
/*FARSI*/        LINE,
/*FARSI*///*****************************************************************************
/*FARSI*/// RRN (Retrival Reference Number)
/*FARSI*///*****************************************************************************
/*FARSI*/        IF, FLD_RRN,
/*FARSI*/            RGHT, 
/*FARSI*/            FLD_F, FLD_RRN, FC_TRIM,
/*FARSI*/            IF, CND_ROLL, TXT, Rrn, ENDIF,
/*FARSI*/        ENDIF,
/*FARSI*/        LINE,
/*FARSI*/
/*FARSI*/  ELSE, // LNG_FARSI %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
/*FARSI*/
/*FARSI*///*****************************************************************************
/*FARSI*/// AVS (Address Verification System) response
/*FARSI*///*****************************************************************************
/*FARSI*/        IF, FLD_AVSRSP,
/*FARSI*/            RGHT,
/*FARSI*/            FLD_F, FLD_AVSRSP, FC_TRIM,LINE,
/*FARSI*/            ' ', TXT, Avs,
/*FARSI*/        ENDIF,
/*FARSI*/  ELSE, // LNG_FARSI %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
/*FARSI*/
/*FARSI*///*****************************************************************************
/*FARSI*/// Host print text
/*FARSI*///*****************************************************************************
/*FARSI*/        IF, FLD_HSTPRDATA,
/*FARSI*/            IF, CND_ROLL, 
/*FARSI*/                LINE, 
/*FARSI*/            ENDIF,
/*FARSI*/            RGHT, FLD, FLD_HSTPRDATA,
/*FARSI*/            IF, CND_ROLL, 
/*FARSI*/                LINE, 
/*FARSI*/            ENDIF,
/*FARSI*/            LINE,
...
/*FARSI*/
/*FARSI*///*****************************************************************************
/*FARSI*/// Transaction amounts
/*FARSI*///*****************************************************************************
/*FARSI*/
/*FARSI*///--------------------------------
/*FARSI*/// Balance enquiry transaction:
/*FARSI*/        IF, TRN_BALI,
/*FARSI*/            LINE, LINE,
/*FARSI*/            IF, CND_ROLL, 
/*FARSI*/                DOUBLE, 
...
/*FARSI*/                ENDIF,
/*FARSI*/                FLD, FLD_TDBAL,
/*FARSI*/                RGHT, ' ', TXT, PclToday,
/*FARSI*/            ENDIF,
/*FARSI*/            LINE,
/*FARSI*/        ELSE,
/*FARSI*/
/*FARSI*///--------------------------------
/*FARSI*/// Base amount for Refund transaction only:
/*FARSI*/            IF, TRN_REFD,
/*FARSI*/                IF, FLD_TAX, OR, FLD_TIP,
/*FARSI*/                    FLD_S, FLD_AMT1, 4, LINE,
/*FARSI*/                    RGHT, ' ', TXT, Base,
/*FARSI*/                ENDIF,
/*FARSI*///--------------------------------
/*FARSI*/// Base amount for other transactions:
/*FARSI*/            ELSE,
/*FARSI*/                IF, CND_TIP, OR, FLD_TAX, OR, FLD_AMT3,
/*FARSI*/                    FLD_S, FLD_AMT1, 4, LINE,
/*FARSI*/                    RGHT, ' ', TXT, Base,
/*FARSI*/                ENDIF,
/*FARSI*/            ENDIF,
/*FARSI*///--------------------------------
/*FARSI*/// Tax amount (not BalInq)
/*FARSI*/            IF, FLD_TAX,
/*FARSI*/                FLD_S, FLD_TAX, 4, LINE,
/*FARSI*/                RGHT, ' ', TXT, Tax,
/*FARSI*/            ENDIF,
/*FARSI*///--------------------------------
/*FARSI*/// Bar amount (not BalInq)
/*FARSI*/            IF, CND_ROLL,
/*FARSI*/                IF, FLD_AMT3,
/*FARSI*/                    FLD_S, FLD_AMT3, 4, LINE,
/*FARSI*/                    RGHT, ' ', TXT, Bar,
/*FARSI*/                ENDIF,
/*FARSI*/            ENDIF,
/*FARSI*/
/*FARSI*///--------------------------------
/*FARSI*/// Restaurant processing amounts:
/*FARSI*/            IF, CND_TIP,
/*FARSI*/                IF, CND_ROLL,
/*FARSI*/// Cash Back amount
/*FARSI*/                    IF, FLD_CASH,
/*FARSI*/                        FLD, FLD_CASH,
/*FARSI*/                        RGHT, ' ', TXT, CashBack, LINE,
/*FARSI*/                    ENDIF,
/*FARSI*/// FEE amount
/*FARSI*/                    IF, FLD_FEE,
/*FARSI*/                        FLD, FLD_FEE,
/*FARSI*/                        RGHT, ' ', TXT, Fee, LINE,
/*FARSI*/                    ENDIF,
/*FARSI*/// Merchant Surcharge amount
/*FARSI*/                    IF, FLD_MSURC,
/*FARSI*/                        FLD_S, FLD_MSURC, 4,
/*FARSI*/                        RGHT, ' ', TXT, Surcharge, LINE,
/*FARSI*/                    ENDIF,
/*FARSI*/                ENDIF,
/*FARSI*/
/*FARSI*/// Tip amount
/*FARSI*/                IF, FLD_TIP,
/*FARSI*/                    FLD_S, FLD_TIP, 4,
/*FARSI*/                ENDIF,
/*FARSI*/                RGHT, ' ', 
/*FARSI*/                IF, NOT, TRN_REFD,
/*FARSI*/                    TXT, Tip,
/*FARSI*/                ELSE,
/*FARSI*/                    IF, FLD_TIP,
/*FARSI*/                        TXT, Tip,
...
/*FARSI*/// TOTAL amount
/*FARSI*/                IF, NOT, TRN_REFD, 
/*FARSI*/                    RGHT, MLTCHR, 9, '_', 
/*FARSI*/                    LINE, 
/*FARSI*/                ENDIF,
/*FARSI*/                
/*FARSI*/                IF, FLD_TIP, OR, TRN_REFD, 
/*FARSI*/                    FLD_S, FLD_TOTAL, 4, 
/*FARSI*/                ENDIF,
/*FARSI*/
/*FARSI*/                IF, CND_ROLL, 
/*FARSI*/                    DOUBLE,
/*FARSI*/                ENDIF,
/*FARSI*/                RGHT, ' ', TXT, Total, 
/*FARSI*/// Suggested Tips guide                
/*FARSI*/                IF, NOT, FLD_TIP, AND, NOT, TRN_REFD, AND, NOT, TRN_VOID,
/*FARSI*/                    IF, FLD_SUGTIP1, OR, FLD_SUGTIP2, OR, FLD_SUGTIP3,
/*FARSI*/                        LINE, LINE, CNTR,
/*FARSI*/                        TXT, TipGuide,
/*FARSI*/                        LINE,
/*FARSI*/                    ENDIF,
/*FARSI*/                    CNTR,
/*FARSI*/                    IF, FLD_SUGTIP1,
/*FARSI*/                        FLD_F, FLD_TIPPCT1, FN_TRIM, '%', '=',
/*FARSI*/                        FLD_F, FLD_SUGTIP1, FA_TRIM, ' ', ' ',
/*FARSI*/                    ENDIF,
/*FARSI*/                    IF, FLD_SUGTIP2,
/*FARSI*/                        FLD_F, FLD_TIPPCT2, FN_TRIM, '%', '=',
/*FARSI*/                        FLD_F, FLD_SUGTIP2, FA_TRIM, ' ', ' ',
/*FARSI*/                    ENDIF,
/*FARSI*/                    IF, FLD_SUGTIP3,
/*FARSI*/                        FLD_F, FLD_TIPPCT3, FN_TRIM, '%', '=',
/*FARSI*/                        FLD_F, FLD_SUGTIP3, FA_TRIM, ' ', ' ',
/*FARSI*/                    ENDIF,
/*FARSI*/                ENDIF,
/*FARSI*/            
/*FARSI*/            
/*FARSI*///--------------------------------
/*FARSI*/// NON-Restaurant processing amounts:
/*FARSI*/            ELSE,
/*FARSI*/                IF, FLD_CASH, OR, FLD_FEE, OR, FLD_MSURC,
/*FARSI*/// Sale amount
/*FARSI*/                    IF, FLD_AMT1,
/*FARSI*/                        FLD, FLD_AMT1,
/*FARSI*/                        //RGHT, ' ', TXT, SaleAmt, LINE,
/*FARSI*/                        RGHT, ' ', TXT, SaleAmt, LINE,
/*FARSI*/                    ENDIF,
/*FARSI*/// Cash amount
/*FARSI*/                    IF, FLD_CASH,
...
/*FARSI*/                ENDIF,
                        CNTR,
/*FARSI*/                FLD, FLD_TOTF, // FA_TRIM,
/*FARSI*/                //IF, FLG_TCONF, TCOPT2, TC2_PRTTOT,
/*FARSI*/                //    DOUBLE, RGHT, TXT, Total,
/*FARSI*/                //ENDIF,
/*FARSI*/            ENDIF,
/*FARSI*/        ENDIF,
/*FARSI*/        LINE,
...
/*FARSI*/    IF, FLD_AMTDUE,
/*FARSI*/        LINE, DOUBLE,
/*FARSI*/        FLD_F, FLD_AMTDUE, FA_TRIM,
/*FARSI*/        RGHT, TXT, AmountDue, LINE,
/*FARSI*/    ENDIF,
/*FARSI*/  ELSE, // LNG_FARSI %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        IF, FLD_AMTDUE,
            LINE, DOUBLE,
...
/*FARSI*/  IF, LNG_FARSI, //%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
/*FARSI*/    IF, FLD_DCD1,
/*FARSI*/        RGHT, FLD, FLD_DCD1, '0', ' ', FLD, FLD_DTX1,   
/*FARSI*/        LINE,
/*FARSI*/    ENDIF,
/*FARSI*/  ELSE, // LNG_FARSI %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        IF, FLD_DCD1,
...
/*FARSI*/    IF, CND_ROLL,
/*FARSI*/        IF, FLD_DCD2,
/*FARSI*/            RGHT, FLD, FLD_DCD2, '0', ' ', FLD, FLD_DTX2, LINE,  
/*FARSI*/        ENDIF,
/*FARSI*/    ELSEIF, NOT, CND_LNGE, 20,
/*FARSI*/        IF, FLD_DCD2,
/*FARSI*/            RGHT, FLD, FLD_DCD2, '0', ' ', FLD, FLD_DTX2,  
/*FARSI*/        ENDIF,
/*FARSI*/        LINE,
/*FARSI*/    ENDIF,
/*FARSI*/  ELSE, LNG_FARSI, //%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
//*****************************************************************************
/*FARSI*/  IF, LNG_FARSI, //%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
/*FARSI*/    IF, NOT, CND_ROLL, AND, FLG_ISS, ISOPT3, IS3_NOREFPRT,
/*FARSI*/        RGHT, TXT, NoRefundsAllow,
/*FARSI*/    ELSEIF, FLD_DCD3,
/*FARSI*/        RGHT, FLD, FLD_DCD3,'0',' ',FLD, FLD_DTX3,   
/*FARSI*/    ELSEIF, TRN_OFFL,
/*FARSI*/        RGHT, TXT, OfflineEntered,
/*FARSI*/    ENDIF,
/*FARSI*/  ELSE, LNG_FARSI, //%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        IF, NOT, CND_ROLL, AND, FLG_ISS, ISOPT3, IS3_NOREFPRT,
            TXT, NoRefundsAllow,
...
/*FARSI*/    IF, CND_ROLL,
/*FARSI*/        IF, FLD_DCD4,
/*FARSI*/            RGHT, FLD, FLD_DCD4, '0', ' ', FLD, FLD_DTX4, LINE,
/*FARSI*/        ENDIF,
/*FARSI*/    ELSEIF, NOT, CND_LNGE, 23,
/*FARSI*/        IF, FLD_DCD4,
/*FARSI*/            RGHT, FLD, FLD_DCD4, '0', ' ', FLD, FLD_DTX4,
/*FARSI*/        ENDIF,
/*FARSI*/        LINE,
/*FARSI*/    ENDIF,
/*FARSI*/  ELSE, LNG_FARSI, //%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
const UBYTE ALIGN(4) RP_TEST_DATA[] = {
    ...
    ELSE,
        ...
        IF, LNG_FARSI,
            LINE,
            TXT, Normal,LINE,
            TXT, S_ABCDEFGHIJ, LINE,
            TXT, S_KLMNOPQRST, LINE,
            TXT, S_UVWXYZ, LINE,
            TXT, S_abcdefghi, LINE,
            TXT, S_klmnopqrst, LINE,
            TXT, S_uvwxyz0123, LINE,
            TXT, S_SetSymbol1, LINE,
            TXT, S_SetSimbols2, LINE,
            LINE,

            DOUBLE, TXT, Double, LINE,
            DOUBLE, TXT, S_ABCDEFGHIJ, LINE,
            DOUBLE, TXT, S_KLMNOPQRST, LINE,
            DOUBLE, TXT, S_UVWXYZ, LINE,
            DOUBLE, TXT, S_abcdefghi, LINE,
            DOUBLE, TXT, S_klmnopqrst, LINE,
            DOUBLE, TXT, S_uvwxyz0123, LINE,
            DOUBLE, TXT, S_SetSymbol1, LINE,
            DOUBLE, TXT, S_SetSimbols2, LINE,
            LINE,
/*
            FONT, LARGE_FONT, 'L','A','R','G','E',LINE,
            FONT, LARGE_FONT, TXT, S_ABCDEFGHIJ, LINE,
            FONT, LARGE_FONT, TXT, S_KLMNOPQRST, LINE,
            FONT, LARGE_FONT, TXT, S_UVWXYZ, LINE,
            FONT, LARGE_FONT, TXT, S_abcdefghi, LINE,
            FONT, LARGE_FONT, TXT, S_klmnopqrst, LINE,
            FONT, LARGE_FONT, TXT, S_uvwxyz0123, LINE,
            FONT, LARGE_FONT, TXT, S_SetSymbol1, LINE,
            FONT, LARGE_FONT, TXT, S_SetSimbols2, LINE,
            LINE,
*/
...
const UBYTE ALIGN(4) RP_AUDIT_DATA[] = {
    ...
    IF, CND_FIRST,              ///**** HEADING LINES *************
        ...
        TXT, AuditReport,
        RGHT, FLD_F, FLD_SYSDT, FD_JEJALI_TEXT,// FD_TEXT, 
        LINE,
...
#ifdef MAKE_FARSI
        IF, LNG_FARSI,
            LFT, FLD_F, FLD_TID, FC_TRIM, RGHT,
        ENDIF,
#endif // MAKE_FARSI
