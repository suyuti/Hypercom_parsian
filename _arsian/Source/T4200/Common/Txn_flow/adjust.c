...
void Adjust( void )
{
	...
	if ( InputPswdAdjust(  ) )
	{
		// Okay; get the Cashier Number allowing NULL entry
		if ( InputCashier( CustomMsg, True ) )
		{
			// Okay; perform the Adjust with no transaction selected
			DoAdjust ( );
		}
	}
}
...
//-----------------------------------------------------------------------------
//!  \brief
//!     Adds up the Amounts and stores a new
//!     TRINP.TRTOTAM, that is if there is no overflow
//!
//!  \param
//!     pSaveAmount pointer to saved amount.
//!
//!  \return
...
//!
//!  \return
//!     Bool        True  - Password entered okay or may skip it
...
//-----------------------------------------------------------------------------
//!  \brief
//!     If a Tip Amount does not exist, and we have Tip
...
//-----------------------------------------------------------------------------
//!  \brief
//!     Confirm the Tip as computed from the entered Total
//!
//!  \param
//!     None
//!
//!  \return
//!     UBYTE       0 - cancel/abort the Transaction
//!                 1 - okay to continue
...
//-----------------------------------------------------------------------------
//!  \brief
//!     Get and save the Adjust Base Amount
//!
//!  \param
//!     None
//!
...
//-----------------------------------------------------------------------------
//!  \brief
//!     Get and save the Adjust Bar Amount
//!
//!  \param
//!     None
//!
...
static Bool InputAdjBar( void )
{
	...
	// Do we prompt for the Bar Amount ? 
...
//-----------------------------------------------------------------------------
//!  \brief
//!     Get and save the Adjust Tax Amount
//!
//!  \param
//!     None
//!
...
//-----------------------------------------------------------------------------
//!  \brief
//!     Get and savt the Adjust Tip Amount
//!
//!  \param
//!     None
//!
...
static Bool InputAdjTip( void )
{
	...
	// Do we prompt for the Tip Amount ? 
	if ( !IsRest(  ) )
	{
		// No; return okay 
		return ( True );
...
//-----------------------------------------------------------------------------
//!  \brief
//!     All the new Totals have been entered, now it is
//!     time to store the changes in the Journal
//!
//!  \param
//!     AdjIdent    Transaction Journal Identifier
//!
//!  \return
//!     Bool        True  - okay to continue
//!                 False - cancel/abort the transaction.
...
//-----------------------------------------------------------------------------
//!  \brief
//!     Should the Tip be prompted for first in Adjust ?
//!
//!  \param
//!     None
//!
...
static Bool CondFirstTip( void )
{
	...
	// Are we in Restaurant Mode ? 
	if ( IsRest(  ) )
	{
		// Yes; is this a Refund Transaction ? 
		if ( REFUND != TRINP.TRKEY )
		{
			// No; add Concord Host items here 

			// Has a Tip been entered ? 
			if ( !( TRINP.TRSTATUS[0] & TS1_TIPENT ) )
			{
				// No; set Prompt flag 
				PromptFlag = True;
			}
