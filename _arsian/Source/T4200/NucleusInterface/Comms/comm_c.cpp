...
// This define should be included to prevent compilation errors on T4100
#ifndef _INCLUDE_NUCLEUS_NET_
...
//!
ThreadTransport* ThreadTransport::GetThreadInstance( ThreadTransport* this_ )
{
	static ThreadTransport* curObj = NULL;
...
//-----------------------------------------------------------------------------
//!
//!      Inspires the communications task to complete its execution.
//!
//! \param
//!      None
//!
...
{
    m_event.SendToFront( HypCTransport::SHUTDOWN_REQUEST );
}
...
void ThreadTransport::RecvMessage_()
{
    ...
    if ( NULL != TRREQ.TRAQPTR )
    {
        // for acquirer transaction
        timeout = CvtBin( TRREQ.TRAQPTR->AQTOUT );
    }
...
bool ThreadTransport::WaitEvent()
{
	while( true )
	{
		...
		case HypCTransport::SHUTDOWN_REQUEST:
            m_state= (States)-1;    // lets Execute() completes
			return true;
...
//! \note
//!
HypETransportError ThreadTransport::SetDialConfig()
{
	...
	unsigned char *pn;
	HypSAddress address;
	HypCTransportAddressSet trAddress;

	switch( m_tranMode )
	{
		case SETTLE_TRAN:
			// Get primary/secondary dial params for settlement
			AddDialUp( trAddress, address, &m_paq->conn.dial.settle );
			break;

		case COMMON_TRAN:
			// Get primary/secondary dial params for tarns
			AddDialUp( trAddress, address, &m_paq->conn.dial.trans );
			break;
	...
	m_Transport.SetDestination( trAddress );
	m_timeOfConnect = 0;
	for( int i = 0; i < trAddress.AddressCount( ); i++ )
	{
		const HypSAddress* addr = trAddress.GetNextAddress();
		m_timeOfConnect += ( addr->retries * ( addr->timeout + 41000 ) );
	}
...
//!
bool ThreadTransport::AddIP( HypSAddress& address, struct ipcontab *pIp, int timeout )
{
...
//!		true on success otherwise flase
//!
bool ThreadTransport::AddDialUp( HypCTransportAddressSet& trAddress, HypSAddress& address, PS_DIAL_STRUCT* dial )
{
	//connection wait time
	int cw;
	//dial attempts
	...
	unsigned char *pn;
	char phoneStr[25];
	PHONE_STRUCT* phone = reinterpret_cast<PHONE_STRUCT*>(dial);

	// Get primary/secondary dial params
	for( int i = 0; i < 2; i++ )
	{	
		GetDialParam( cw, da, pn, phone );
		if( ( pn[0] != 0xFF ) && ( da != 0 ) )
		{
			address.retries = da;
