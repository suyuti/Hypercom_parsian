...
void TerminateApplicationTasks( HDTPROCESS *entryPointTable )
{
    ...
    // Stopping application tasks
    for( nProcIndex = 0; nProcIndex < (int)NumAppTasks; nProcIndex++ )
    {
        if( entryPointTable[nProcIndex].pShutdown )
            entryPointTable[nProcIndex].pShutdown();
    }
